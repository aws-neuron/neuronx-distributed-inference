#!/usr/bin/env python3
"""
Integration tests for llava-v1.5-7b NeuronX implementation.
"""
...
from modeling_llava_neuron import NeuronLlavaForCausalLM, LlavaInferenceConfig

# Test configuration
MODEL_PATH = "/home/ubuntu/models/llava-v1.5-7b/"
COMPILED_MODEL_PATH = "/home/ubuntu/neuron_models/llava-v1.5-7b/"
...
def compiled_model():
    ...
    if not (compiled_path / "model.pt").exists():
        ...
        config = LlavaInferenceConfig(neuron_config, load_config=load_pretrained_config(MODEL_PATH))
        model = NeuronLlavaForCausalLM(MODEL_PATH, config)
    ...
    try:
        model_config = LlavaInferenceConfig.from_pretrained(MODEL_PATH, neuron_config=neuron_config)
    except:
        model_config = LlavaInferenceConfig(neuron_config, load_config=load_pretrained_config(MODEL_PATH))
    
    try:
        model = NeuronLlavaForCausalLM.from_pretrained(COMPILED_MODEL_PATH, config=model_config)
    except:
        model = NeuronLlavaForCausalLM(MODEL_PATH, model_config)
...
if __name__ == "__main__":
    print("llava-v1.5-7b Integration Tests")
    print("="*80)
    # Run tests...
