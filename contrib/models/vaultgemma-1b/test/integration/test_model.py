...
from modeling_vaultgemma import NeuronVaultGemmaForCausalLM, VaultGemmaInferenceConfig
...
def compiled_model():
    ...
    if not (compiled_path / "model.pt").exists():
        ...
        config = VaultGemmaInferenceConfig(neuron_config, load_config=load_pretrained_config(MODEL_PATH))
    ...
    try:
        model_config = VaultGemmaInferenceConfig.from_pretrained(MODEL_PATH, neuron_config=neuron_config)
    except:
        model_config = VaultGemmaInferenceConfig(neuron_config, load_config=load_pretrained_config(MODEL_PATH))
    
    try:
        model = NeuronVaultGemmaForCausalLM.from_pretrained(COMPILED_MODEL_PATH, config=model_config)
    except:
        model = NeuronVaultGemmaForCausalLM(MODEL_PATH, model_config)
