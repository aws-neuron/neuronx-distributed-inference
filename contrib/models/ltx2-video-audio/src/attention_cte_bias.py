...
logger = get_logger("attention_cte_bias")
...
def _attention_cte_impl(
    ...
    for section_idx in range(atp.num_sections):
        ...
        # Initialize ones buffer for bias broadcast via nc_matmul outer product
        if ac.has_bias:
            nisa.memset(bufs.ones_sb, 1.0)
        ...
        # Load additive bias tiles for this batch item (if bias is provided)
        if ac.has_bias:
            for kt in range(atp.num_k_tiles_per_section):
                k_start = sp.section_offset + kt * _K_TILE_SZ
                num_f = min(ac.seqlen_k_active - k_start, _K_TILE_SZ)
                if k_start < ac.seqlen_k_active and num_f > 0:
                    nisa.dma_copy(
                        dst=bufs.bias_sb[kt][0, :num_f],
                        src=bias[batch_id, k_start : k_start + num_f],
                    )
...
class AttnInternalBuffers(nl.NKIObject):
    ...
    bias_sb = None  # additive bias tiles loaded to SBUF [_K_TILE_SZ tiles]
    ones_sb = None  # all-ones vector [1, sb_p] for bias broadcast via nc_matmul
...
def _allocate_attention_buffers(
    ...
    # Allocate bias SBUF tiles: [1, _K_TILE_SZ] per K tile in section.
    # Bias is 1D over K positions, broadcast across Q partition dim.
    # Also allocate ones_sb [1, sb_p] for bias broadcast via nc_matmul outer product.
    if ac.has_bias:
        bufs.bias_sb = allocator.alloc_sbuf_tensor(
            shape=(1, _K_TILE_SZ),
            dtype=nl.float32,
            block_dim=[atp.num_k_tiles_per_section],
            num_free_tiles=[atp.num_k_tiles_per_section],
        )
        bufs.ones_sb = allocator.alloc_sbuf_tensor(
            shape=(1, atp.sb_p),
            dtype=nl.float32,
        )
...
def _load_q_tile(
    ...
    if tp_q:
        ...
        if grps_per_load > 1:
            ...
            tp_dt = load_dtype
            psum_buf = nl.ndarray(
                (d, _Q_GRP_SZ), dtype=tp_dt, buffer=nl.psum, address=(0, 0)
            )
    ...
    if tp_k:
        ...
        for tile in range(num_tiles):
            ...
            if use_dma_tp:
                ...
                tp_dt = load_dtype
                psum_buf = nl.ndarray(
                    (d, num_pe_tps, sb_p), dtype=tp_dt, buffer=nl.psum, address=(0, 0)
                )
    ...
    for k_tile_idx in range(num_k_tiles_in_large_tile):
        ...
        if matmul_selection and k_tile_idx_in_section < atp.num_k_tiles_per_section:
            ...
            if diagonal_sel_mask:  # static diagonal mask
                ...
                if ac.has_bias:
                    # Add additive bias to QK^T scores via nc_matmul PSUM accumulation.
                    # nc_matmul computes: mm1_psum_tile += ones_sb.T @ bias_sb
                    # = outer product [num_p, 1] x [1, num_f] = [num_p, num_f]
                    # This broadcasts the 1D bias across all Q positions (partition dim).
                    nisa.nc_matmul(
                        mm1_psum_tile[:num_p, :num_f],
                        bufs.ones_sb[:1, :num_p],
                        bufs.bias_sb[k_tile_idx_in_section][:1, :num_f],
                    )
                nisa.tensor_scalar_reduce(
                    mm1_masked_tile[:num_p, nl.ds(k_tile_idx * _K_TILE_SZ, num_f)],
