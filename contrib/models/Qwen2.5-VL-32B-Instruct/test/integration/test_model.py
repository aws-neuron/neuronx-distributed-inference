#!/usr/bin/env python3
"""
Integration tests for Qwen2.5-VL-32B-Instruct NeuronX implementation.
"""
...
from modeling_qwen2vl import NeuronQwen2VLForConditionalGeneration, Qwen2VLInferenceConfig

# Test configuration
MODEL_PATH = "/home/ubuntu/models/Qwen2.5-VL-32B-Instruct/"
COMPILED_MODEL_PATH = "/home/ubuntu/neuron_models/Qwen2.5-VL-32B-Instruct/"
...
def compiled_model():
    ...
    if not (compiled_path / "model.pt").exists():
        ...
        config = Qwen2VLInferenceConfig(neuron_config, load_config=load_pretrained_config(MODEL_PATH))
        model = NeuronQwen2VLForConditionalGeneration(MODEL_PATH, config)
    ...
    try:
        model_config = Qwen2VLInferenceConfig.from_pretrained(MODEL_PATH, neuron_config=neuron_config)
    except:
        model_config = Qwen2VLInferenceConfig(neuron_config, load_config=load_pretrained_config(MODEL_PATH))
    
    try:
        model = NeuronQwen2VLForConditionalGeneration.from_pretrained(COMPILED_MODEL_PATH, config=model_config)
    except:
        model = NeuronQwen2VLForConditionalGeneration(MODEL_PATH, model_config)
...
if __name__ == "__main__":
    print("Qwen2.5-VL-32B-Instruct Integration Tests")
    print("="*80)
    # Run tests...
