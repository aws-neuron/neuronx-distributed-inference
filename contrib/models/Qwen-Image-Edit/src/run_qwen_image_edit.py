...
# Set SPMD world size. Default 8 (V3 CP/CFG world=8). Override via QIE_WORLD_SIZE for
# larger CP (e.g. TP=4 x CP=4 = world 16). Must match the transformer NEFF's world_size.
_WORLD = int(os.environ.get("QIE_WORLD_SIZE", str(TP_DEGREE)))
os.environ["LOCAL_WORLD_SIZE"] = str(_WORLD)
...
print(f"Neuron runtime configured: world={_WORLD}, LNC=2")
...
MODEL_ID = os.environ.get("QIE_MODEL_PATH", "Qwen/Qwen-Image-Edit-2509")
SEED = 42
...
class NeuronTransformerWrapperV3CP(torch.nn.Module):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None,
        ...
        _profile_step = int(_os.environ.get("QIE_PROFILE_STEP", "-1"))
        if _profile_step >= 0:
        ...
        if _should_profile:
            ...
            print(f">>> QIE V3 CP profile START (step {self._step_count}, dump={_dump})", flush=True)
        ...
        # This ensures CP split results in sequences aligned for NKI Flash Attention.
        # Zero-padding is fine here (verified at CP=2 and CP=4 with correct outputs); the
        # pad patches' contribution is negligible vs real tokens in the (unmasked) attention.
        ...
        if _should_profile:
            ...
            print(f">>> QIE V3 CP profile STOP", flush=True)

        _step_dt = _time.time() - _step_t0
        if not hasattr(self, "_step_times"):
            self._step_times = []
        self._step_times.append(_step_dt)
        print(f"  [transformer_step {len(self._step_times)}] {_step_dt*1000:.1f} ms", flush=True)
...
def patch_pipeline_for_cfg_parallel(pipe):
    ...
    def __call__(
        ...
        _qie_stage_times["vae_encode"] = _time.time() - _ve_t0
        print(f"  [vae_encode] {_qie_stage_times['vae_encode']*1000:.1f} ms (encodes {len(vae_images)} input image(s))", flush=True)
        img_shapes = [
            [
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if do_true_cfg:
                    # If the wrapped transformer was compiled with batch=1
                    # (e.g. V3 CP NEFF), run CFG sequentially as two batch=1
                    # forwards. Otherwise (V3 CFG NEFF), batch neg+pos into a
                    # single batch=2 forward and split.
                    cb = getattr(self.transformer, "compiled_batch_size", 2)
                    if cb < 2:
                        # Sequential CFG: two batch=1 calls
                        ts1 = t.expand(latents.shape[0]).to(latents.dtype) / 1000
...
class NeuronVAEWrapper(torch.nn.Module):
    ...
    def _tiled_encode(self, x):
        ...
        When the compiled NEFF has batch_size > 1, tiles are gathered into a
        single batched call instead of one NEFF launch per tile. For an input
        that produces N tiles and a compiled batch size B:
          - if N <= B: 1 NEFF call with batch padding
          - if N > B: ceil(N/B) NEFF calls with B tiles each
        Each NEFF launch pays a ~37 ms overhead, so collapsing 6 launches into
        1 saves ~5×37 ms ≈ 185 ms per input image.
        """
        b, c, t, h, w = x.shape
        ...
        # Phase 1: enumerate all tiles, padding edge tiles to expected size.
        tile_inputs = []
        tile_meta = []  # (row_idx, col_idx, actual_h, actual_w)
        row_idx = 0
        for i in range(0, h, self.tile_sample_stride_height):
            ...
            for j in range(0, w, self.tile_sample_stride_width):
                ...
                tile_meta.append((row_idx, col_idx, actual_h, actual_w))
                col_idx += 1
            row_idx += 1
        n_rows = row_idx
        n_cols = max(m[1] for m in tile_meta) + 1

        # Phase 2: run encoder in chunks of compiled_batch_size.
        # Each tile already has batch dim 1; cat along dim 0 to form the batch.
        encoded_flat = []
        bs = self.compiled_batch_size
        for chunk_start in range(0, len(tile_inputs), bs):
            chunk = tile_inputs[chunk_start:chunk_start + bs]
            batched = torch.cat(chunk, dim=0)  # (n_chunk, c, t, eh, ew)
            encoded = self._encode_tile(batched)  # NEFF call; pads to bs internally
            # _encode_tile already strips batch padding; encoded has n_chunk samples
            for k in range(len(chunk)):
                encoded_flat.append(encoded[k:k+1])

        # Phase 3: scatter into 2D grid and crop back to non-padded sizes.
        rows = [[None] * n_cols for _ in range(n_rows)]
        for (ri, ci, actual_h, actual_w), enc in zip(tile_meta, encoded_flat):
            if actual_h < self.expected_height or actual_w < self.expected_width:
                crop_h = actual_h // self.spatial_compression_ratio
                crop_w = actual_w // self.spatial_compression_ratio
                enc = enc[:, :, :, :crop_h, :crop_w]
            rows[ri][ci] = enc

        # Blend tiles together (unchanged from per-tile path)
        result_rows = []
    ...
    def _tiled_decode(self, z):
        ...
        When the compiled NEFF has batch_size > 1, all tiles are gathered into
        chunks of compiled_batch_size and the decoder NEFF is invoked once per
        chunk instead of once per tile (mirrors the _tiled_encode path).
        """
        b, c, t, latent_h, latent_w = z.shape
        ...
        # Phase 2: run decoder in chunks of compiled_batch_size.
        decoded_flat = []
        bs = self.compiled_batch_size
        for chunk_start in range(0, len(tile_inputs), bs):
            chunk = tile_inputs[chunk_start:chunk_start + bs]
            batched = torch.cat(chunk, dim=0)
            decoded = self._decode_tile(batched)
            for k in range(len(chunk)):
                decoded_flat.append(decoded[k:k+1])

        # Phase 3: scatter into 2D grid and crop padded tiles.
        rows = [[None] * n_cols for _ in range(n_rows)]
        for (ri, ci, actual_h, actual_w), dec in zip(tile_meta, decoded_flat):
            if actual_h < tile_latent_h or actual_w < tile_latent_w:
                crop_h = actual_h * self.spatial_compression_ratio
                crop_w = actual_w * self.spatial_compression_ratio
                dec = dec[:, :, :, :crop_h, :crop_w]
            rows[ri][ci] = dec
...
def load_all_compiled_models(compiled_models_dir: str, pipe, args):
    ...
    if use_v3_cfg:
        ...
        # Reuse the CFG-parallel pipeline patch for its stage-timing markers.
        # The no-CFG branch of the patched __call__ is identical in signature
        # to upstream and works with the V3 CP wrapper.
        patch_pipeline_for_cfg_parallel(pipe)
    elif use_v2_flash:
    ...
    # Resolve portrait NEFF dims for the vision encoder. Prefer the values written
    # into vision_encoder_v3/config.json by compile_vision_encoder_v3.py; fall back
    # to CLI args / image_size for legacy single-device or TP=8 vision encoders.
    if vision_encoder_config is not None:
        wrapper_image_h = vision_encoder_config.get("image_h",
                          vision_encoder_config.get("image_size", args.image_size))
        wrapper_image_w = vision_encoder_config.get("image_w",
                          vision_encoder_config.get("image_size", args.image_size))
    else:
        wrapper_image_h = getattr(args, "image_h", None) or args.image_size
        wrapper_image_w = getattr(args, "image_w", None) or args.image_size
    print(f"  Vision encoder grid: {wrapper_image_h}x{wrapper_image_w} "
          f"({wrapper_image_h//14}x{wrapper_image_w//14} patches)")
    ...
    pipe.text_encoder = NeuronTextEncoderWrapper(
        ...
        image_h=wrapper_image_h,
        image_w=wrapper_image_w,
...
def run_inference(args):
    ...
    # Force the VLM (condition) branch to use our exact compiled grid (image_h x image_w).
    # The pipeline normally calls `calculate_dimensions(CONDITION_IMAGE_SIZE, src_ratio)`
    # which rounds to multiples of 32 from the *source* aspect ratio — that does NOT
    # land on our portrait NEFF (e.g. 336x448) when the source ratio differs even slightly.
    # Patch the symbol used inside the pipeline module so it returns our fixed (W, H)
    # regardless of input ratio when the call is for CONDITION_IMAGE_SIZE.
    vlm_h = getattr(args, 'image_h', None) or args.image_size
    vlm_w = getattr(args, 'image_w', None) or args.image_size
    qwen_pipeline_module.CONDITION_IMAGE_SIZE = vlm_h * vlm_w
    _orig_calc_dims = qwen_pipeline_module.calculate_dimensions
    _condition_target = vlm_h * vlm_w
    _vae_target = compiled_vae_pixels
    def _patched_calc_dims(target_area, ratio):
        if target_area == _condition_target:
            return vlm_w, vlm_h
        if target_area == _vae_target:
            return args.width, args.height
        return _orig_calc_dims(target_area, ratio)
    qwen_pipeline_module.calculate_dimensions = _patched_calc_dims
    print(f"  Forcing condition (VLM) dims to {vlm_w}x{vlm_h} regardless of source ratio")
    print(f"  Forcing VAE dims to {args.width}x{args.height} regardless of source ratio")
    ...
    # Qwen2VLImageProcessorFast.preprocess() reads from `size["shortest_edge"]` /
    # `size["longest_edge"]` (NOT from min_pixels/max_pixels attributes alone), so we must
    # set both. Even with these set, smart_resize will produce the closest
    # multiple-of-(patch_size*merge_size)=28 grid that fits the constraints — since the
    # condition branch above pre-resizes images to exactly (vlm_w, vlm_h) which is already
    # a multiple of 28, smart_resize is a no-op.
    vis_h = getattr(args, 'image_h', None) or args.image_size
    vis_w = getattr(args, 'image_w', None) or args.image_size
    target_pixels = vis_h * vis_w
    print(f"\nConfiguring processor for vision encoder size: {vis_w}x{vis_h} (W x H)")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image_size", type=int, default=448,
                        help="Vision encoder square image size (must match compiled model). "
                             "Ignored if --image_h and --image_w are set.")
    parser.add_argument("--image_h", type=int, default=None,
                        help="Vision encoder image height in pixels (portrait NEFF). "
                             "Must match the value used at compile time.")
    parser.add_argument("--image_w", type=int, default=None,
                        help="Vision encoder image width in pixels (portrait NEFF). "
                             "Must match the value used at compile time.")
