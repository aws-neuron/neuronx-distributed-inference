...
def compile_vision_encoder_v3(args):
    ...
    world_size = int(os.environ.get("VISION_WORLD_SIZE", 8))  # 4 for TP=4 CP=1, 8 for TP=4 CP=2

    image_h = args.image_h if args.image_h else args.image_size
    image_w = args.image_w if args.image_w else args.image_size
    patch_size = 14
    temporal_patch_size = 2
    spatial_merge_size = 2

    for name, dim in (("image_h", image_h), ("image_w", image_w)):
        if dim % patch_size != 0:
            raise ValueError(
                f"{name} ({dim}) must be divisible by patch_size ({patch_size}). "
                f"Valid sizes: 224, 336, 448, 560, etc."
            )
        if (dim // patch_size) % spatial_merge_size != 0:
            raise ValueError(
                f"{name} / patch_size ({dim // patch_size}) must be divisible by "
    ...
    print("=" * 60)
    print(f"  Image size (HxW): {image_h}x{image_w}")
    print(f"  Patch size: {patch_size}")
    print(f"  Num patches (HxW): {num_patches_h}x{num_patches_w} = {num_patches}")
    ...
    with NxDParallelState(world_size=world_size, tensor_model_parallel_size=tp_degree):
        ...
        config = {
            ...
            "image_size": image_h if image_h == image_w else max(image_h, image_w),
            "image_h": image_h,
            "image_w": image_w,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=448,
        help="Vision encoder input image size when square (default: 448). "
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Vision encoder input image height (in pixels). Overrides --image_size for height.",
    )
    parser.add_argument(
        "--image_w",
        type=int,
        default=None,
        help="Vision encoder input image width (in pixels). Overrides --image_size for width.",
    )
    parser.add_argument(
        "--compiled_models_dir",
        type=str,
