...
def nki_flash_attention(query, key, value):
    ...
    # The LNC2 sharded flash kernel requires seqlen_q divisible by a power of 2 >= 512.
    # TP=8 keeps the FULL query (not CP-split), so q_len = joint seq (e.g. 12944) which
    # is not aligned. Pad ONLY the query seqlen up to the next multiple of 512 (key/value
    # stay full, so padding never pollutes real tokens — pad queries just produce junk rows
    # we slice off). q is [B*H, D, q_len]; pad the last dim.
    vc_size = int(os.getenv("NEURON_RT_VIRTUAL_CORE_SIZE", "1"))
    ALIGN = 512
    q_pad = ((q_len + ALIGN - 1) // ALIGN) * ALIGN
    if vc_size == 2 and q_pad != q_len:
        pad_n = q_pad - q_len
        q = torch.nn.functional.pad(q, (0, pad_n))  # pad last dim (seqlen_q) of [B*H, D, q_len]
    else:
        q_pad = q_len

    attn_output = torch.zeros((bs * n_head, q_pad, d_head), dtype=torch.bfloat16, device=q.device)
    ...
    # Slice off the padded query rows
    if attn_output.shape[1] != q_len:
...
class NKIQwenAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if os.getenv("QIE_DROP_CM", "0") == "1":
            # drop_cm sparse attention (TP=8 symmetric => no merge): drop cloth<->model
            # cross-attention. joint layout [txt, noise, cloth, model], img = 3 frames of F.
            # query groups: (txt+noise) see ALL; cloth see [txt,noise,cloth]; model see [txt,noise,model].
            # Each group is ONE full softmax over its own KV (verified bit-exact, no merge).
            S_img = img_query.shape[2]
            F = S_img // 3                       # base_patches per frame
            st = seq_txt
            nz_end = st + F                      # txt+noise end
            cl_end = st + 2 * F                  # cloth end
            # KV slices
            k_all, v_all = joint_key, joint_value
            k_tnc = joint_key[:, :, :cl_end, :]; v_tnc = joint_value[:, :, :cl_end, :]   # [txt,noise,cloth]
            # [txt,noise,model] = gather (drop cloth band)
            k_tnm = torch.cat([joint_key[:, :, :nz_end, :], joint_key[:, :, cl_end:, :]], dim=2)
            v_tnm = torch.cat([joint_value[:, :, :nz_end, :], joint_value[:, :, cl_end:, :]], dim=2)
            # query groups
            q_tn = joint_query[:, :, :nz_end, :]      # txt+noise
            q_cl = joint_query[:, :, nz_end:cl_end, :] # cloth
            q_md = joint_query[:, :, cl_end:, :]       # model
            o_tn = nki_flash_attention(q_tn, k_all, v_all)
            o_cl = nki_flash_attention(q_cl, k_tnc, v_tnc)
            o_md = nki_flash_attention(q_md, k_tnm, v_tnm)
            joint_hidden_states = torch.cat([o_tn, o_cl, o_md], dim=2)
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path", type=str, default=None,
                        help="Path to model (local dir or HF ID). If not set, uses MODEL_ID with CACHE_DIR")
    ...
    # Override MODEL_ID/CACHE_DIR if model_path provided (matches V3 CP)
    if args.model_path:
