...
from nkilib.core.utils.kernel_helpers import PSUM_BANK_SIZE, div_ceil, get_verified_program_sharding_info
...
def _attention_cte_impl(
    ...
    # === HOISTED Q: allocate Q once with all-resident tiles (no rotation) ===
    # Original layout: Q allocated inside section loop with num_free_tiles=[2],
    # forcing reload at the start of each section. With QIE's num_sections=2,
    # the second section reloaded all 13 Q-tiles redundantly.
    #
    # Hoisted layout: Q gets num_free_tiles = block_dim (all 13 alive), allocated
    # before the section loop. Q is preloaded once. _allocate_attention_buffers
    # is called with skip_q_alloc=True from the section loop so it doesn't
    # clobber the Q address.
    bufs.q_sb = allocator.alloc_sbuf_tensor(
        ...
        num_free_tiles=[div_ceil(atp.num_grps, atp.num_q_grps_per_load)],  # all alive
        align_to=32,
    )

    # Mark allocator checkpoint for section-level reset (now AFTER Q allocation)
    sbuf_addr_outer = allocator.get_current_address()

    # Preload all Q-load groups once (covers all sections).
    # Use a temp SectionParams (section_idx 0 / offset 0) — the q load impl
    # only reads ac, atp, q, batch_id; SectionParams isn't actually consulted
    # for Q load (only used by the function for `is_causal` predicate).
    _q_preload_sp = SectionParams(
        section_idx=0,
        section_offset=0,
        section_offset_active=0,
        next_section_offset_active=0,
        section_contains_prefix=False,
        next_section_contains_prefix=False,
    )
    for _q_grp in range(0, atp.num_grps, atp.num_q_grps_per_load):
        _load_q_impl(_q_grp, ac, atp, _q_preload_sp, bufs, q, batch_id, sbuf_addr_outer)
    ...
    for section_idx in range(atp.num_sections):
        ...
        # Allocate the internal buffers (skip Q: hoisted above the section loop)
        allocator.set_current_address(sbuf_addr_outer)
        _allocate_attention_buffers(allocator, ac, atp, bufs, sink, skip_q_alloc=True)
        ...
        # Start actual compute. Q is already preloaded above the section loop —
        # all _load_q_impl calls inside the original code are skipped here.
        if shard_seqlen_q_length <= 1:
            ...
            for grp_i in range(shard_seqlen_q_start, shard_seqlen_q_end - 2):  # for each block of seq_q
                ...
                # grp_i   :  PV, write_back
                # grp_i+1 :  EXP
                # grp_i+2 :  QK+Max  (Q already loaded above)
...
def _allocate_attention_buffers(
    ...
    skip_q_alloc: If True, skip Q SBUF allocation. Used by the QIE-tuned variant
    where Q is allocated once before the section loop with num_free_tiles equal
    to its block_dim, so all Q tiles stay alive across sections.
    """
