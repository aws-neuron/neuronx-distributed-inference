...
class NeuronTextEncoderWrapper(nn.Module):
    ...
    def __init__(self, original_text_encoder, compiled_vision_encoder=None,
                 ...
                 image_h=None, image_w=None,  # Portrait NEFF: vision encoder H, W (overrides image_size)
        ...
        # Portrait/landscape NEFF support: vision encoder may have been compiled
        # with non-square dims (image_h != image_w). When provided, these override
        # the square assumption.
        self.image_h = image_h if image_h is not None else image_size
        self.image_w = image_w if image_w is not None else image_size
        self.grid_h = self.image_h // self.patch_size
        self.grid_w = self.image_w // self.patch_size
        self.num_patches_per_image = self.grid_h * self.grid_w

        # Calculate expected dimensions
        self.num_image_tokens = (self.grid_h // self.spatial_merge_size) * (self.grid_w // self.spatial_merge_size)
    ...
    def forward(self, input_ids=None, attention_mask=None, pixel_values=None,
        ...
        _vis_t0 = _time2.time() if _os2.environ.get("QIE_STAGE_TIMING", "0") == "1" else None
        if pixel_values is not None:
            ...
            # Option 2: Use V3 Vision Encoder (TP=4, NxDModel, float32, fast)
                ...
                # Compiled vision encoder NEFF expects fixed (grid_h * grid_w) patches.
                # For portrait NEFF (image_h != image_w), grid is non-square.
                expected_patches_per_image = self.num_patches_per_image  # grid_h * grid_w
                ...
                if num_images > 1:
                    ...
                    for img_idx in range(num_images):
                        ...
                        # Create grid_thw for single image (matches compiled NEFF grid)
                        single_grid_thw = torch.tensor([[1, self.grid_h, self.grid_w]], dtype=torch.int64)
                    ...
                    if actual_patches != expected_patches_per_image:
                        ...
                        image_grid_thw = torch.tensor([[1, self.grid_h, self.grid_w]], dtype=torch.int64)
        ...
        if _vis_t0 is not None:
            print(f"    [vision_v3] {(_time2.time()-_vis_t0)*1000:.1f} ms", flush=True)
        ...
        if self.use_cpu_language_model:
            ...
            _lm_t0 = _time.time() if _os.environ.get("QIE_STAGE_TIMING", "0") == "1" else None
            hidden_states = self.compiled_language_model_v3(
                inputs_embeds.to(torch.bfloat16),
                attention_mask,
                position_ids
            )
            if _lm_t0 is not None:
                print(f"    [LM_v3] {(_time.time()-_lm_t0)*1000:.1f} ms", flush=True)
