...
BATCH_SIZE=${7:-1}  # Batch size for transformer/language model (for batched inference)

# VAE compile-time batch dimension. For V3 CP / V3 CFG we default to 6 so the
# tiled VAE encoder/decoder can run all 6 tiles of a 1024x1024 image in a
# single NEFF launch (saves ~5x ~37ms launch overhead per image; ~6% E2E).
# The runtime _tiled_encode/_tiled_decode falls back to chunking for any
# tile count, so larger compiled batch is safe.
if [[ "$VERSION_MODE" == "v3_cp" || "$VERSION_MODE" == "v3_cfg" ]]; then
    VAE_BATCH_SIZE=${VAE_BATCH_SIZE:-6}
else
...
echo "VAE Batch Size: ${VAE_BATCH_SIZE}"
echo ""
...
echo "  - vae_encoder/ (tile: ${VAE_TILE_SIZE}x${VAE_TILE_SIZE}, batch: ${VAE_BATCH_SIZE})"
echo "  - vae_decoder/ (tile: ${VAE_TILE_SIZE}x${VAE_TILE_SIZE}, batch: ${VAE_BATCH_SIZE})"
if [[ "$VERSION_MODE" == "all" || "$VERSION_MODE" == "v1" ]]; then
