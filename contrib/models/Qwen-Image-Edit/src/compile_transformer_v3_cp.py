...
# Phase 17: lower the all-reduce dtype on RowParallelLinear from fp32 -> bf16.
# Mirrors the V3 CFG plumbing; halves the bytes on the wire for every TP
# all-reduce inside the transformer blocks. Default on; set
# QIE_ALLREDUCE_BF16=0 to revert.
ALLREDUCE_BF16 = os.getenv("QIE_ALLREDUCE_BF16", "1") == "1"
_REDUCE_DTYPE = torch.bfloat16 if ALLREDUCE_BF16 else torch.float32
if ALLREDUCE_BF16:
    print("  + TP all-reduce dtype = bf16 (Phase 17 experiment)")
...
def compile_transformer_v3_cp(args):
    ...
    if context_parallel_enabled:
        ...
        # NKI Flash Attention requires local seqlen_q to be a multiple of 128. BUT when
        # tp_degree >= 8 each rank has few heads (24/8=3), so the kernel falls back to a
        # SHARDED-attention path that requires seqlen_q divisible by a power of 2 >= 512.
        # Align to 512 in that case (512 satisfies the divisibility); 128 otherwise.
        alignment = 512 if tp_degree >= 8 else 128
    ...
    with NxDParallelState(world_size=world_size, tensor_model_parallel_size=tp_degree):
        ...
        # NOTE: -O2 matches the V3 CFG path (compile_transformer_v3_cfg.py); validated on QIE shapes.
        # QIE_OPT_LEVEL: opt-in override (default O2). Set to "3" to try -O3 (NOTE: known to OOM on QIE V3 CP).
        # QIE_CC_TILING: --cc-pipeline-tiling-factor (default 4). Larger = more comm/compute overlap chunks.
        # NOTE on fp8: --auto-cast=matmult --auto-cast-type=fp8_e4m3 is a NO-OP here — it casts fp32→fp8,
        #   but QIE runs bf16 end-to-end (PyTorch model bf16, AR bf16, attention bf16). Verified
        #   2026-05-28: NEFF compiled with the flag matched baseline at 7.66 s E2E. To actually get fp8,
        #   weights must be explicitly quantized in PyTorch (per-channel scale + calibration) — comparable
        #   engineering scope to writing custom NKI matmul kernels.
        _opt = os.environ.get("QIE_OPT_LEVEL", "2")
        _cc_tiling = os.environ.get("QIE_CC_TILING", "4")
        compile_args = f"--model-type=transformer -O{_opt} --auto-cast=none --lnc=2 --tensorizer-options='--enable-ccop-compute-overlap --cc-pipeline-tiling-factor={_cc_tiling}' --internal-hlo2tensorizer-options='--enable-native-kernel=1 --remat'"
        print(f"  compile_args: {compile_args}")
        # QIE_WLO: weight layout optimization. Passing priority_model_key triggers
        #   ModelBuilder._compile_priority_model -> mark_weights_for_wlo + compile_wlo +
        #   compile_layout_transformer. The layout transformer is embedded in nxd_model.pt and
        #   applied at runtime by NxDModel.set_weights (nxd_model.py:301) — so disk weights stay
        #   in normal layout and QIE_SKIP_WEIGHTS=1 fast-recompile still picks up the new layout.
        #   bit-exact (only reshapes weight storage, no numerical change). Default on; =0 to revert.
        _wlo = os.environ.get("QIE_WLO", "1") == "1"
        _priority_key = "inference" if _wlo else None
        if _wlo:
            print("  + weight layout optimization (priority_model_key='inference')")
        traced_model = builder.compile(
            priority_model_key=_priority_key,
        ...
        # QIE_SKIP_WEIGHTS=1: only re-emit nxd_model.pt (the NEFF) and exit.
        # Lets compile-flag experiments (CC tiling, opt level) reuse existing sharded weights.
        if os.environ.get("QIE_SKIP_WEIGHTS", "0") == "1":
            print("\nQIE_SKIP_WEIGHTS=1 — skipping weight sharding/config/rope save.")
            print(f"Updated NEFF only at: {output_path}/nxd_model.pt")
            return
