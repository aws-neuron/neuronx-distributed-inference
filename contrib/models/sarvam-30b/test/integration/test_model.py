#!/usr/bin/env python3
"""
Integration tests for sarvam-30b NeuronX implementation.

This model uses the Sarvam MoE architecture with:
- 19 layers (1 dense + 18 MoE), 128 routed experts + 1 shared expert, top-6
- Sigmoid routing with learned expert_bias (post-sigmoid, pre-topk)
- Routed scaling factor (2.5x on normalized routing weights)
- Q/K normalization (RMSNorm on head_dim=64)
- Fused QKV with GQA (64 heads, 4 KV heads)
...
from modeling_sarvam import NeuronSarvamForCausalLM, SarvamInferenceConfig


# Test configuration - UPDATE THESE PATHS for your environment
MODEL_PATH = "/shared/dhwanw2/models/sarvam-30b/"
COMPILED_MODEL_PATH = "/tmp/neuron_models/sarvam-30b/"
...
def test_output_coherence(compiled_model, tokenizer):
    ...
    prompt = "To make a cup of coffee, first"
    inputs = tokenizer(prompt, return_tensors="pt", padding=True)
...
if __name__ == "__main__":
    print("=" * 80)
    print("sarvam-30b Integration Tests")
    print("=" * 80)

    compiled_path = Path(COMPILED_MODEL_PATH)
    neuron_config = MoENeuronConfig(
        tp_degree=8, batch_size=1, seq_len=128,
