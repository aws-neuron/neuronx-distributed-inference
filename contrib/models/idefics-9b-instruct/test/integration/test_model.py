#!/usr/bin/env python3
"""
Integration tests for idefics-9b-instruct NeuronX implementation.
"""
...
# Test configuration
MODEL_PATH = "/home/ubuntu/models/idefics-9b-instruct/"
COMPILED_MODEL_PATH = "/home/ubuntu/neuron_models/idefics-9b-instruct/"
...
def compiled_model():
    ...
    if not (compiled_path / "model.pt").exists():
        ...
        config = IdeficsInferenceConfig(neuron_config, load_config=load_pretrained_config(MODEL_PATH))
        model = NeuronIdeficsForCausalLM(MODEL_PATH, config)
    ...
    try:
        model_config = IdeficsInferenceConfig.from_pretrained(MODEL_PATH, neuron_config=neuron_config)
    except:
        model_config = IdeficsInferenceConfig(neuron_config, load_config=load_pretrained_config(MODEL_PATH))
    
    try:
        model = NeuronIdeficsForCausalLM.from_pretrained(COMPILED_MODEL_PATH, config=model_config)
    except:
        model = NeuronIdeficsForCausalLM(MODEL_PATH, model_config)
...
if __name__ == "__main__":
    print("idefics-9b-instruct Integration Tests")
    print("="*80)
    # Run tests...
