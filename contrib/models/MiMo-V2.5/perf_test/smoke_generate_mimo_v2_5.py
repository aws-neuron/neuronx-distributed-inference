#!/usr/bin/env python3
"""Minimal generate smoke test for MiMo-V2.5 FP8 on Trn2.

Assumes the compiled NEFF already exists at MIMO_V2_5_COMPILED_PATH
(from smoke_compile_mimo_v2_5.py). Rebuilds the same MoENeuronConfig /
MiMo-V2.5 wrapper, loads with skip_warmup=False, and generates 20 tokens for a
...
MODEL_PATH = os.environ.get(
    "MIMO_V2_5_MODEL_PATH",
    "/opt/dlami/nvme/models/MiMo-V2.5-Neuron-FP8",
)
COMPILED_PATH = os.environ.get(
    "MIMO_V2_5_COMPILED_PATH",
    "/opt/dlami/nvme/compiled/mimo_v2_5_tp64_moetp1_ep64_fp8/",
)

# Must match smoke_compile_mimo_v2_5.py exactly, else load() sees a
# mismatched NEFF.
...
def main():
    ...
    # world_size = tp_degree; see smoke_compile_mimo_v2_5.py comment).
