...
def compiled_model():
    ...
    if not (compiled_path / "model.pt").exists():
        ...
        config = Phi3InferenceConfig(neuron_config, load_config=load_pretrained_config(MODEL_PATH))
    ...
    try:
        model_config = Phi3InferenceConfig.from_pretrained(MODEL_PATH, neuron_config=neuron_config)
    except:
        model_config = Phi3InferenceConfig(neuron_config, load_config=load_pretrained_config(MODEL_PATH))
    
    try:
        model = NeuronPhi3ForCausalLM.from_pretrained(COMPILED_MODEL_PATH, config=model_config)
    except:
        model = NeuronPhi3ForCausalLM(MODEL_PATH, model_config)
