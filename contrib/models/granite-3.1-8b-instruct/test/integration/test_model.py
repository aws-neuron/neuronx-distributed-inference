...
# Test configuration
MODEL_PATH = "/home/ubuntu/models/granite-3.1-8b-instruct/"
COMPILED_MODEL_PATH = "/home/ubuntu/neuron_models/granite-3.1-8b-instruct/"
...
def compiled_model():
    ...
    if not (compiled_path / "model.pt").exists():
        ...
        config = GraniteInferenceConfig(neuron_config, load_config=load_pretrained_config(MODEL_PATH))
    ...
    try:
        model_config = GraniteInferenceConfig.from_pretrained(MODEL_PATH, neuron_config=neuron_config)
    except:
        model_config = GraniteInferenceConfig(neuron_config, load_config=load_pretrained_config(MODEL_PATH))
    
    try:
        model = NeuronGraniteForCausalLM.from_pretrained(COMPILED_MODEL_PATH, config=model_config)
    except:
