...
"""
Attention Block TKG Kernel — Multi-KV-Head Fork

Forked from nkilib.experimental.transformer.attention_block_tkg to support
kv_heads_per_rank > 1 (e.g., 8 KV heads at TP=4 = 2 KV heads per rank).

The original kernel hardcodes kv_heads=1. This version:
  - Derives kv_heads from the W_qkv weight shape
  - Processes K with n_heads=kv_heads (not 1)
  - Loops over KV head groups for attention, calling attention_tkg once per group
  - Accumulates partial O-proj results across groups
  - Updates all KV cache heads

The inner attention kernel (attention_tkg) is unchanged — it already operates on
one KV head group at a time.

Single kernel call per layer → avoids the NCC_ITEN404 compiler ICE that occurs
with the multi-call monkeypatch approach.
"""
...
from nkilib.core.qkv.qkv import qkv_tkg
...
# KVDP sharding helpers — removed for multi-KV-head kernel.
# KVDP is NOT supported with kv_heads_per_rank > 1.
# The KVDP parameter is kept in the API for compatibility but asserted == 1.
...
def attention_block_tkg(
    ...
    # -- rmsnorm X
    rmsnorm_X_enabled: bool = None,
    rmsnorm_X_eps: Optional[float] = None,
    rmsnorm_X_gamma: Optional[nl.ndarray] = None,
    # -- qkv projections
    W_qkv: nl.ndarray = None,
    bias_qkv: Optional[nl.ndarray] = None,
    quantization_type_qkv: QuantizationType = None,
    weight_dequant_scale_qkv: Optional[nl.ndarray] = None,
    input_dequant_scale_qkv: Optional[nl.ndarray] = None,
    # -- Q/K processing: pre-RoPE RMSNorm
    rmsnorm_QK_pre_rope_enabled: bool = None,
    rmsnorm_QK_pre_rope_eps: float = None,
    rmsnorm_QK_pre_rope_W_Q: Optional[nl.ndarray] = None,
    rmsnorm_QK_pre_rope_W_K: Optional[nl.ndarray] = None,
    # -- Q/K processing: RoPE
    cos: Optional[nl.ndarray] = None,
    sin: Optional[nl.ndarray] = None,
    rope_contiguous_layout: bool = None,
    # -- Q/K processing: post-RoPE RMSNorm
    rmsnorm_QK_post_rope_enabled: bool = None,
    rmsnorm_QK_post_rope_eps: float = None,
    rmsnorm_QK_post_rope_W_Q: Optional[nl.ndarray] = None,
    rmsnorm_QK_post_rope_W_K: Optional[nl.ndarray] = None,
    # -- attention
    K_cache_transposed: bool = None,
    active_blocks_table: Optional[nl.ndarray] = None,
    K_cache: nl.ndarray = None,
    V_cache: nl.ndarray = None,
    attention_mask: nl.ndarray = None,
    sink: Optional[nl.ndarray] = None,
    softmax_scale: Optional[float] = None,
    # -- KV cache update
    update_cache: bool = None,
    kv_cache_update_idx: Optional[nl.ndarray] = None,
    k_scale: Optional[nl.ndarray] = None,
    v_scale: Optional[nl.ndarray] = None,
    # -- output projection
    W_out: Optional[nl.ndarray] = None,
    bias_out: Optional[nl.ndarray] = None,
    quantization_type_out: QuantizationType = None,
    weight_dequant_scale_out: Optional[nl.ndarray] = None,
    input_dequant_scale_out: Optional[nl.ndarray] = None,
    transposed_out: bool = None,
    # -- output
    out_in_sb: bool = None,
    sbm: Optional[SbufManager] = None,
    skip_attention: bool = False,
    # -- Multi-KV-head support
    n_kv_heads: int = 1,
    # -- Number of query heads per rank (passed explicitly to avoid .shape on PlaceholderParameter weights)
    n_q_heads: int = 8,
    # -- Head dimension (passed explicitly to avoid NKI .shape limitations on KV cache tensors)
    head_dim: int = 128,
    # -- Max context length for KV cache (passed explicitly for multi-KV-head)
    s_max_ctx: int = 256,
    # -- Per-group attention mask (for multi-KV-head, avoids NKI reshape-on-slice)
    # Shape: [S_ctx, B, q_per_kv_group, S_tkg] — same mask reused for all KV head groups
    group_attention_mask: Optional[nl.ndarray] = None,
    # -- Pre-allocated HBM buffer for V active tokens (multi-KV-head)
    # Shape: [B_virt, 1, S_tkg, d_head] — pre-allocated by adapter to avoid NCC_IBIR440
    # The compiler's DRAM allocator cannot handle kernel-internal HBM ndarray allocations.
    v_active_hbm: Optional[nl.ndarray] = None,
    # -- Replicated kv_cache_update_idx for multi-KV-head cache update
    # Shape: [B_virt, 1] where B_virt = B * kv_heads — same position replicated per head
    # Pre-allocated by adapter to avoid kernel-internal HBM allocation (NCC_IBIR440)
    kv_cache_update_idx_virt: Optional[nl.ndarray] = None,
):
    """
    ...
    Dimensions:
        ...
        kv_heads: Number of KV heads per rank (derived from W_qkv shape). Supports kv_heads >= 1.
    ...
    # Inline config extraction instead of calling _validate_and_extract_config,
    # because the NKI tracer cannot handle mixed nl.ndarray + Python int function args.

    kv_heads = n_kv_heads if n_kv_heads is not None else 1

    # Extract batch/sequence dimensions from attention_mask (always works in NKI tracer)
    _, B, _, _ = attention_mask.shape

    if X.buffer == nl.sbuf:
        kernel_assert(X.shape[0] == nl.tile_size.pmax, "SBUF input X dim0 must be pmax")
        H = X.shape[2] * nl.tile_size.pmax
        S_tkg = X.shape[1] // B
    else:
        B, S_tkg, H = X.shape

    # d_head from explicit parameter (avoids .shape on 4D KV cache which NKI can't trace)
    d_head = (
        head_dim if head_dim is not None else V_cache.shape[2]
    )  # fallback: 3D cache
    # Compute I from explicit q_heads + kv_heads (avoids W_qkv.shape on PlaceholderParameter)
    q_heads = n_q_heads
    I = d_head * (q_heads + 2 * kv_heads)
    half_d = d_head // 2

    is_KVDP = False  # KVDP not supported with multi-KV-head
    B_attn = B
    ...
    K_cache, V_cache, cache_had_head_dim = __internal_squeeze_head_dim(
        K_cache, V_cache, is_block_kv, kv_heads
    )
    ...
    if is_block_kv:
        ...
        # Always use explicit s_max_ctx to avoid .shape on PlaceholderParameter V_cache
        S_max_ctx = s_max_ctx
        blk_len = 0
    ...
    q_per_kv_group = q_heads // kv_heads  # Q heads per KV head group
    ...
    # ========== Extract V from QKV ==========
    # For multi-KV-head: extract kv_heads * d_head worth of V data
    # V lives at offset q_heads*d + kv_heads*d in the QKV buffer
    # V_tkg_sb shape: [B*S_tkg, kv_heads*d_head] — ALL KV heads
    v_offset = d_head * (q_heads + kv_heads)
    v_size = kv_heads * d_head
    V_tkg_sb = nl.ndarray(
        (B * S_tkg, v_size),
        dtype=QKV_tkg_sb.dtype,
        buffer=nl.sbuf,
        name="attention_blk_V_tkg_sb",
    )
    nisa.tensor_copy(V_tkg_sb, QKV_tkg_sb[:, nl.ds(v_offset, v_size)])
    ...
    # NOTE: KVDP with multi-KV-head not yet supported
    kernel_assert(
        not is_KVDP or kv_heads == 1, "KVDP with multi-KV-head not yet supported"
    )

    # ========== Attention Computation (Virtual Batch Approach) ==========
    # Instead of calling attention_tkg N times (once per KV head group),
    # we reshape the data so all groups become "virtual batches" and call
    # attention_tkg ONCE. This avoids compiler name collisions (NCC_INLA001)
    # that occur when the same sub-kernel is called multiple times.
    #
    # Virtual batch: B_virt = B_attn * kv_heads
    # Each virtual batch has q_per_kv_group Q heads and 1 KV head.
    # Q_tkg_sb layout: [d, B*q_heads*S] = [d, B*(kv*qpg)*S] = [d, B_virt*qpg*S] ✓ (compatible)
    # K_tkg_sb layout: [d, B*kv_heads*S] = [d, B_virt*S] ✓ (compatible)
    # K_cache: [B, kv_heads, d, S_max] → reshape to [B_virt, 1, d, S_max]
    # V_cache: [B, kv_heads, S_max, d] → reshape to [B_virt, 1, S_max, d]
    # group_mask: [S_ctx, B_virt, qpg, S_tkg] (pre-created by adapter)

    # --- For multi-KV-head: copy V tokens to pre-allocated HBM buffer ---
    # V_tkg_sb: [B*S_tkg, kv_heads*d_head] @ SBUF — needs reshaping to virtual batch layout.
    # SBUF partition constraints prevent reshape from [1, 256] to [2, 128] directly.
    # DMA copy to the pre-allocated v_active_hbm (passed from adapter) to avoid NCC_IBIR440.
    if kv_heads > 1:
        B_virt = B_attn * kv_heads
        kernel_assert(
            v_active_hbm is not None,
            "v_active_hbm must be provided when n_kv_heads > 1",
        )
        # DMA copy V from SBUF to HBM.
        # Reshape HBM to match SBUF layout [B*S, kv*d] for the copy.
        nisa.dma_copy(
            v_active_hbm.reshape((B_attn * S_tkg, kv_heads * d_head)),
            V_tkg_sb,
        )
        V_active_hbm = v_active_hbm
    ...
    if skip_attention:
        ...
        _softmax_scale = (
            softmax_scale if softmax_scale is not None else d_head ** (-0.5)
        )
        nisa.tensor_scalar(
        ...
        if kv_heads == 1:
            ...
            # Single-head: V_active on HBM
            V_active_dtype = nl.float8_e4m3 if kv_quant else V_tkg_sb.dtype
            V_active_hbm = nl.ndarray(
                (B_attn, 1, S_tkg, d_head),
                dtype=V_active_dtype,
                buffer=nl.shared_hbm,
                name="v_active_virt_hbm",
            )
            nisa.dma_copy(V_active_hbm.reshape((B_attn * S_tkg, d_head)), V_tkg_sb)

        # --- Prepare KV cache for attention ---
        if is_block_kv:
            k_prior_virt = K_cache
            v_prior_virt = V_cache
        else:
            if kv_heads == 1:
                ...
                k_prior_virt = K_cache.reshape(k_shape)
                v_prior_virt = V_cache.reshape((B_attn, 1, S_max_ctx, d_head))
            else:
                # Multi-KV-head: reshape 4D cache to virtual batch format.
                # K_cache: [B, kv_heads, d, S_max] or [B, kv_heads, S_max, d]
                # V_cache: [B, kv_heads, S_max, d]
                # Target:  [B_virt, 1, ...] where B_virt = B * kv_heads
                #
                # For B=1: K_cache [1, 2, d, S] -> reshape to [2, 1, d, S] — just a reshape!
                # The data layout is [head0_data, head1_data] which maps to
                # [vbatch0_data, vbatch1_data] with the same stride.
                if K_cache_transposed:
                    k_prior_virt = K_cache.reshape((B_virt, 1, d_head, S_max_ctx))
                else:
                    k_prior_virt = K_cache.reshape((B_virt, 1, S_max_ctx, d_head))
                v_prior_virt = V_cache.reshape((B_virt, 1, S_max_ctx, d_head))

        # --- Attention mask ---
        if kv_heads > 1:
            kernel_assert(
                group_attention_mask is not None,
                "group_attention_mask required when n_kv_heads > 1",
            )
            attn_mask = group_attention_mask  # [S_ctx, B_virt, qpg, S_tkg]
        else:
            attn_mask = attention_mask  # [S_ctx, B, q_heads, S_tkg]

        # --- Allocate attention output ---
        attn_out = sbm.alloc_stack(
            (d_head, B_virt * q_per_kv_group * S_tkg), dtype=X.dtype, buffer=nl.sbuf
        )

        attn_cfg = AttnTKGConfig(
            bs=B_virt,
            q_head=q_per_kv_group,
            s_active=S_tkg,
            curr_sprior=S_ctx,
        ...
        attention_tkg(
            q=Q_tkg_sb,  # [d, B_virt * qpg * S] @ SBUF
            k_active=K_tkg_sb,  # [d, B_virt * S] @ SBUF
            v_active=V_active_hbm,  # [B_virt, 1, S, d] @ HBM
            k_prior=k_prior_virt,  # [B_virt, 1, d, S_max] or [B_virt, 1, S_max, d]
            v_prior=v_prior_virt,  # [B_virt, 1, S_max, d]
            mask=attn_mask,
            out=attn_out,
            cfg=attn_cfg,
            sbm=sbm,
    ...
    # NOTE: KVDP not supported with multi-KV-head (asserted above)

    # ========== KV Cache Update ==========
    # For multi-KV-head: K_tkg_sb is [d, B*kv_heads*S] and V_tkg_sb is [B*S, kv_heads*d]
    # We loop over heads and call _kv_cache_update per head with 3D cache slices
    if update_cache:
        if kv_heads == 1:
            # Original single-head path
            _kv_cache_update(
            ...
            # Multi-KV-head: inline cache update using .ap() on 3D cache views.
            #
            # CRITICAL FIX: The original _update_flat_cache uses .ap() on 3D tensors
            # [B, S_max, d] with indirect_dim=1 pointing to the sequence dimension.
            # The previous code reshaped to 2D [B_virt*S_max, d], which changes the
            # dimension numbering — indirect_dim=1 then points to d_head (columns)
            # instead of the sequence position, causing writes to the WRONG location.
            #
            # Fix: reshape 4D cache [B, kv_heads, S_max, d] to 3D [B_virt, S_max, d]
            # so the .ap() semantics match the original exactly.
            #
            # K_cache: [B, kv_heads, S_max, d] or [B, kv_heads, d, S_max] → 3D
            # V_cache: [B, kv_heads, S_max, d] → 3D [B_virt, S_max, d]
            # K_tkg_sb: [d, B*kv_heads*S] @ SBUF
            # V_tkg_sb: [B*S, kv_heads*d] @ SBUF

            B_virt_cache = B_attn * kv_heads

            _, n_prgs, prg_id = get_verified_program_sharding_info(
                "multi_kv_cache_update", (0, 1), 2
            )

            start_position = nl.ndarray((1, 1), dtype=nl.uint32, buffer=nl.sbuf)

            # V update on lnc=0
            # V_cache: [B, kv_heads, S_max, d] → reshape to 3D [B_virt, S_max, d]
            # Then use the SAME .ap() pattern as _update_flat_cache: 3D with indirect_dim=1
            if n_prgs == 1 or prg_id == 0:
                ...
                for b in range(B_attn):
                    ...
                    for g in range(kv_heads):
                        vb = b * kv_heads + g
                        V_col_sb = nl.ndarray(
                            (S_tkg, d_head), dtype=V_tkg_sb.dtype, buffer=nl.sbuf
                        )
                        nisa.tensor_copy(
                            V_col_sb,
                            V_tkg_sb[
            ...
            if n_prgs == 1 or prg_id == 1:
                for b in range(B_attn):
                    ...
                    for g in range(kv_heads):
                        vb = b * kv_heads + g
                        k_src_col = b * kv_heads * S_tkg + g * S_tkg
                        if K_cache_transposed:
                            # K_cache: [B, kv_heads, d, S_max] → 3D [B_virt, d, S_max]
                            K_cache_3d = K_cache.reshape(
                                (B_virt_cache, d_head, S_max_ctx)
                            )
                            K_col_sb = nl.ndarray(
                                (d_head, S_tkg), dtype=K_tkg_sb.dtype, buffer=nl.sbuf
                            )
                            nisa.tensor_copy(
                                K_col_sb, K_tkg_sb[:, nl.ds(k_src_col, S_tkg)]
                            )
                            nisa.dma_copy(
                                dst=K_cache_3d.ap(
                                    pattern=[[S_max_ctx, d_head], [1, S_tkg]],
                                    offset=vb * d_head * S_max_ctx,
                                    scalar_offset=start_position,
                                    indirect_dim=2,
                                ),
                                src=K_col_sb,
                            ...
                            # K_cache: [B, kv_heads, S_max, d] → 3D [B_virt, S_max, d]
                            K_cache_3d = K_cache.reshape(
                                (B_virt_cache, S_max_ctx, d_head)
                            )
                            K_col_sb = nl.ndarray(
                                (d_head, S_tkg), dtype=K_tkg_sb.dtype, buffer=nl.sbuf
                            )
                            ...
                            K_transposed_sb = nl.ndarray(
                                (S_tkg, d_head), K_tkg_sb.dtype, nl.sbuf
                            )
                            _transpose_sbuf(K_col_sb, K_transposed_sb)
                            nisa.dma_copy(
                                dst=K_cache_3d.ap(
                                    pattern=[[d_head, S_tkg], [1, d_head]],
                                    offset=vb * S_max_ctx * d_head,
            ...
            # K_cache and V_cache are already 4D, no unsqueeze needed
    else:  # No cache update: return new K/V tokens
        if kv_heads == 1:
            K_tkg_hbm = nl.ndarray(
                (d_head, B_attn, S_tkg),
            ...
            # V_tkg_hbm: copy from SBUF [B*S, d] to HBM [B, 1, S, d]
            V_tkg_hbm = nl.ndarray(
                (B_attn, 1, S_tkg, d_head),
                dtype=V_tkg_sb.dtype,
                buffer=nl.shared_hbm,
                name="V_hbm",
            )
            nisa.dma_copy(V_tkg_hbm.reshape((B_attn * S_tkg, d_head)), V_tkg_sb)
        else:
            # Return multi-head K tokens: [d, B, kv_heads, S]
            K_tkg_hbm = nl.ndarray(
                (d_head, B_attn, kv_heads, S_tkg),
                dtype=K_tkg_sb.dtype,
                buffer=nl.shared_hbm,
                name="K_hbm",
            )
            nisa.dma_copy(K_tkg_hbm.reshape(K_tkg_sb.shape), K_tkg_sb)
            # Return multi-head V tokens: [B, kv_heads, S, d]
            # V_tkg_sb is [B*S, kv_heads*d] in SBUF
            V_tkg_hbm = nl.ndarray(
                (B_attn, kv_heads, S_tkg, d_head),
                dtype=V_tkg_sb.dtype,
                buffer=nl.shared_hbm,
                name="V_hbm",
            )
            nisa.dma_copy(
                V_tkg_hbm.reshape((B_attn * S_tkg, kv_heads * d_head)), V_tkg_sb
            )
...
def _validate_and_extract_config(
    ...
    # kv_heads and d_head_param are passed as positional int args (no NKI tracing issues).
    # d_head_param is the head dimension. For multi-KV-head, it must be provided.
    # For single-head, derive from V_cache.shape (3D cache, .shape works).
    d_head = d_head_param
    if d_head == 0 and kv_heads == 1:
        # Fallback for single-head: V_cache is 3D (B, S, d)
        d_head = V_cache.shape[2]
    I = W_qkv.shape[1]

    # KVDP not supported with multi-KV-head kernel
    # Hardcode is_KVDP=False, B_attn=B, q_heads_attn=q_heads
    is_KVDP = False
    kernel_assert(
        B * S_tkg <= nl.tile_size.pmax,
        f"B * S_tkg must be <= {nl.tile_size.pmax}, got {B * S_tkg}",
    )
    kernel_assert(d_head % 2 == 0, f"d_head must be even, got {d_head}")
    kernel_assert(
        d_head > 0 and I % d_head == 0,
        f"QKV weights must be packed as (q_heads + 2*kv_heads) * d_head, got I={I}, d_head={d_head}, kv_heads={kv_heads}",
    )
    ...
    if is_block_kv:
        ...
        S_ctx = attention_mask.shape[0]
        if kv_heads > 1:
            # Multi-KV-head: NKI tracing cannot access .shape on 4D KV cache tensors.
            # Use explicitly passed s_max_ctx parameter.
            kernel_assert(
                s_max_ctx_param is not None,
                "s_max_ctx must be provided when n_kv_heads > 1",
            )
            S_max_ctx = s_max_ctx_param
        else:
            S_max_ctx = V_cache.shape[1]
        blk_len = 0
        # Skip shape validation for multi-KV-head (NKI can't access .shape on 4D cache)
        if kv_heads == 1:
            kernel_assert(
...
def __internal_squeeze_head_dim(
    K_cache: nl.ndarray, V_cache: nl.ndarray, is_block_kv: bool, kv_heads: int
) -> Tuple[nl.ndarray, nl.ndarray, bool]:
    """
    Remove head dimension from 4D cache tensors when kv_heads == 1.
    For multi-KV-head (kv_heads > 1), leave the 4D tensors as-is.

    NKI tracing does not support len(tensor.shape), so we use the kv_heads
    parameter to determine the cache layout.

    Args:
        K_cache (nl.ndarray): Key cache (3D or 4D)
        V_cache (nl.ndarray): Value cache (3D or 4D)
        is_block_kv (bool): Block KV cache flag
        kv_heads (int): Number of KV heads per rank

    Returns:
        Tuple[nl.ndarray, nl.ndarray, bool]: (K_processed, V_processed, had_head_dim)
    """
    if kv_heads > 1:
        # Multi-KV-head: cache is 4D (B, kv_heads, ...), keep as-is
        # NKI tracing cannot access .shape on 4D KV cache tensors
        return K_cache, V_cache, True
    else:
        # kv_heads == 1: cache is 3D from NxDI (B, S, d) or (num_blocks, block_len, d)
        # NxDI with kv_heads==1 always creates 3D caches, so .shape is accessible.
        # Just return as-is (no squeeze needed since it's already 3D).
    ...
    if n_prgs == 1 or prg_id == 1:
        ...
        if K_cache_transposed:
            ...
            nisa.dma_copy(
                ...
                src=K_reshaped_sb.ap(
                    pattern=[[S_tkg * d_head, B], [d_head, S_tkg], [1, d_head]]
                ),
            )
...
def _update_block_cache_batched(
    ...
    kv_cache_update_idx_sb = nl.ndarray(
        kv_cache_update_idx.shape, kv_cache_update_idx.dtype, nl.sbuf
    )
