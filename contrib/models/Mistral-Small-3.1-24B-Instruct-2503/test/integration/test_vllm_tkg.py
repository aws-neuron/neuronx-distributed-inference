"""
Integration test for Mistral-Small-3.1-24B with TKG optimization via vLLM-neuron.

Tests basic model loading and inference via vLLM-neuron server.
Requires: trn2.3xlarge instance with SDK 2.29, LNC=1 for TP=8.
Note: Text model must be extracted first via extract_text_model.py.
"""
...
MODEL_PATH = "/home/ubuntu/models/Mistral-Small-24B-text-only"
TP_DEGREE = 8
MAX_MODEL_LEN = 8192
PORT = 8000
...
def test_smoke():
    ...
    print("Server not running. Extract text model and start vLLM server first.")
    print("Note: LNC=1 is required for TP=8.")
    sys.exit(1)
...
def test_throughput():
    ...
    assert tok_s > 25, f"Throughput too low: {tok_s:.1f} tok/s (expected >25)"
