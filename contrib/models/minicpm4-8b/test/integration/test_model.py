...
from modeling_minicpm import NeuronMiniCPMForCausalLM, MiniCPMInferenceConfig
...
def create_model_for_inference(compiled_path: str, model_path: str):
    ...
    try:
        model_config = MiniCPMInferenceConfig.from_pretrained(
            model_path, neuron_config=neuron_config, trust_remote_code=True
        )
    except (TypeError, AttributeError):
        model_config = MiniCPMInferenceConfig(
            neuron_config, load_config=load_pretrained_config(model_path),
        )
    
    model = NeuronMiniCPMForCausalLM(model_path, model_config)
