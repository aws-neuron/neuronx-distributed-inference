...
# Test configuration
MODEL_PATH = "/home/ubuntu/models/Baichuan2-7B-Base/"
COMPILED_MODEL_PATH = "/home/ubuntu/neuron_models/Baichuan2-7B-Base/"
...
def create_model_for_inference(compiled_path: str, model_path: str):
    ...
    # Create model config
    try:
        model_config = Baichuan2InferenceConfig.from_pretrained(
            model_path, neuron_config=neuron_config,
        )
    except (TypeError, AttributeError):
        model_config = Baichuan2InferenceConfig(
            neuron_config, load_config=load_pretrained_config(model_path),
        )

    # Create model
    try:
        if hasattr(NeuronBaichuan2ForCausalLM, 'from_pretrained'):
            model = NeuronBaichuan2ForCausalLM.from_pretrained(compiled_path, config=model_config)
        else:
            raise AttributeError("No from_pretrained method")
    except (TypeError, AttributeError, Exception):
        model = NeuronBaichuan2ForCausalLM(model_path, model_config)
