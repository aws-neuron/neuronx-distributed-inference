...
"""
NxDI contrib: Qwen3.5-9B (qwen3_5 -- dense model)

Hybrid DeltaNet + Standard Attention + Dense MLP architecture.

24 of 32 layers use Gated DeltaNet (linear recurrent attention)
8 of 32 layers use standard GQA with KV cache + output gate
All 32 layers use a dense SwiGLU MLP (intermediate_size=12288)
...
class NeuronGatedDeltaNet(nn.Module):
    ...
    Replaces standard attention for 24 of 32 layers in Qwen3.5-9B.
    Uses a chunk-based linear recurrence instead of KV cache.

    HF weight layout (9B dense):
    - in_proj_qkv.weight: (key_dim*2 + value_dim, hidden_size) = (8192, 4096)
    - in_proj_z.weight: (value_dim, hidden_size) = (4096, 4096)
    - in_proj_a.weight: (num_v_heads, hidden_size) = (32, 4096)
    - in_proj_b.weight: (num_v_heads, hidden_size) = (32, 4096)
    - conv1d.weight: (conv_dim, 1, conv_kernel_size) = (8192, 1, 4)
    - A_log: (num_v_heads,) = (32,)
    - dt_bias: (num_v_heads,) = (32,)
    - norm.weight: (head_v_dim,) = (128,)
    - out_proj.weight: (hidden_size, value_dim) = (4096, 4096)
    """
...
class Qwen35InferenceConfig(InferenceConfig):
    """Config for Qwen3.5-9B (dense) with hybrid DeltaNet + Attention."""
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "layer_types" not in kwargs and not any(
            ...
            num_layers = kwargs.get("num_hidden_layers", 32)
            if num_layers % 4 != 0:
                raise ValueError(
...
class NeuronQwen35Attention(NeuronAttentionBase):
    ...
    16 Q heads, 4 KV heads (4:1 GQA), head_dim=256 for the 9B dense model.
...
class Qwen35MLP(nn.Module):
    """Dense SwiGLU MLP for Qwen3.5-9B.

    gate_proj: hidden_size -> intermediate_size (4096 -> 12288)
    up_proj:   hidden_size -> intermediate_size (4096 -> 12288)
    down_proj: intermediate_size -> hidden_size (12288 -> 4096)
...
class NeuronQwen35ForCausalLM(NeuronBaseForCausalLM):
    ...
    def convert_hf_to_neuron_state_dict(state_dict, config):
        ...
        if (
            ...
            and "lm_head.weight" not in new_sd
            and "embed_tokens.weight" in new_sd
        ):
