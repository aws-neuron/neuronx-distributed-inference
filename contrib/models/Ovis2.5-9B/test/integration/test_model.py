#!/usr/bin/env python3
"""
Integration tests for Ovis2.5-9B NeuronX implementation.
"""
...
from modeling_ovis2_5 import NeuronOvis2_5_ForCausalLM, Ovis2_5_InferenceConfig

# Test configuration
MODEL_PATH = "/home/ubuntu/models/Ovis2.5-9B/"
COMPILED_MODEL_PATH = "/home/ubuntu/neuron_models/Ovis2.5-9B/"
...
def compiled_model():
    ...
    if not (compiled_path / "model.pt").exists():
        ...
        config = Ovis2_5_InferenceConfig(neuron_config, load_config=load_pretrained_config(MODEL_PATH))
    ...
    try:
        model_config = Ovis2_5_InferenceConfig.from_pretrained(MODEL_PATH, neuron_config=neuron_config)
    except:
        model_config = Ovis2_5_InferenceConfig(neuron_config, load_config=load_pretrained_config(MODEL_PATH))
    
    try:
        model = NeuronOvis2_5_ForCausalLM.from_pretrained(COMPILED_MODEL_PATH, config=model_config)
    except:
        model = NeuronOvis2_5_ForCausalLM(MODEL_PATH, model_config)
...
if __name__ == "__main__":
    print("Ovis2.5-9B Integration Tests")
    print("="*80)
    # Run tests...
