...
class NeuronTalkerModel(NeuronBaseModel):
    ...
    via encode_vision_to_input(). Unlike the upstream NeuronBaseModel which
    only calls encode_vision_to_input during context encoding, this subclass
    overrides get_model_output to also inject during token generation (for
    per-step thinker state injection), matching HF's Qwen2.5-Omni behavior.
    """
    ...
    def get_model_output(
        ...
        **kwargs,
        ...
        """Override to inject thinker states during token generation.

        Upstream gates encode_vision_to_input behind is_for_context_encoding,
        which is correct for Qwen2-VL/Qwen3-VL but wrong for Talker because
        Talker needs per-step thinker state injection during autoregressive
        decode. We pre-inject here, then pass inputs_embeds to super() with
        vision_embeddings=None/vision_mask=None so the parent doesn't try
        to inject again.
        """
        if (
            not is_for_context_encoding
            and vision_embeddings is not None
            and vision_mask is not None
        ):
            # Token-generation phase: compute inputs_embeds ourselves, inject,
            # then let super() skip its vision block.
