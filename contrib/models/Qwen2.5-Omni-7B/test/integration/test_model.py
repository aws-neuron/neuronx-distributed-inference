#!/usr/bin/env python3
"""
Integration tests for Qwen2.5-Omni-7B NeuronX implementation.
"""
...
# Test configuration
MODEL_PATH = "/home/ubuntu/models/Qwen2.5-Omni-7B/"
COMPILED_MODEL_PATH = "/home/ubuntu/neuron_models/Qwen2.5-Omni-7B/"
...
def compiled_model():
    ...
    if not (compiled_path / "model.pt").exists():
        ...
        config = Qwen2_5OmniInferenceConfig(neuron_config, load_config=load_pretrained_config(MODEL_PATH))
    ...
    try:
        model_config = Qwen2_5OmniInferenceConfig.from_pretrained(MODEL_PATH, neuron_config=neuron_config)
    except:
        model_config = Qwen2_5OmniInferenceConfig(neuron_config, load_config=load_pretrained_config(MODEL_PATH))
    
    try:
        model = NeuronQwen2_5OmniForCausalLM.from_pretrained(COMPILED_MODEL_PATH, config=model_config)
    except:
        model = NeuronQwen2_5OmniForCausalLM(MODEL_PATH, model_config)
...
if __name__ == "__main__":
    print("Qwen2.5-Omni-7B Integration Tests")
    print("="*80)
    # Run tests...
