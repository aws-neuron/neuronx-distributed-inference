#!/usr/bin/env python3
"""Integration tests for minicpm-moe-8x2b NeuronX implementation."""
...
MODEL_PATH = "/shared/dhwanw2/models/MiniCPM-MoE-8x2b/"
COMPILED_MODEL_PATH = "/home/dhwanw/workplace/neuronx-distributed-inference/contrib/models/minicpm-moe-8x2b/compiled_model/"
...
def create_model_for_inference(compiled_path: str, model_path: str):
    ...
    try:
        model_config = MiniCPMMoEInferenceConfig.from_pretrained(model_path, neuron_config=neuron_config)
    except (TypeError, AttributeError):
        model_config = MiniCPMMoEInferenceConfig(neuron_config, load_config=load_pretrained_config(model_path))
    try:
        model = NeuronMiniCPMMoEForCausalLM.from_pretrained(compiled_path, config=model_config)
    except:
        model = NeuronMiniCPMMoEForCausalLM(model_path, model_config)
