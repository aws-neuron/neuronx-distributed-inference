"""Partial MiniMax-M3-MXFP8 compile + TTFT/ITL on Trn2.

Loads the first N layers from the MXFP8 quantized checkpoint, dequantizes
MXFP8 → BF16 during state_dict loading, and runs end-to-end on Neuron.
"""
...
MODEL_PATH = "/mnt/scratch/models/MiniMax-M3-MXFP8"
COMPILED_PATH = "/mnt/scratch/neuron_models/MiniMax-M3-MXFP8"
NUM_LAYERS = int(os.environ.get("M3_NUM_LAYERS", "6"))
...
# Propagate quantization_config so the converter can read the block size.
if "quantization_config" in raw:
    text_cfg["quantization_config"] = raw["quantization_config"]
...
print(f"\nConfig: num_hidden_layers={config.num_hidden_layers}, "
      ...
      f"quantization_config={'present' if hasattr(config, 'quantization_config') else 'absent'}")
...
print("\n[load] loading weights to Neuron (includes MXFP8 dequant)...")
t0 = time.perf_counter()
model = NeuronMiniMaxM3ForCausalLM(MODEL_PATH, config)
