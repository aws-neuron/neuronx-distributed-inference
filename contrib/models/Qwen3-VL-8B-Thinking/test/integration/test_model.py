...
from modeling_qwen3_vl import NeuronQwen3VLForCausalLM, Qwen3VLInferenceConfig
...
def compiled_model():
    ...
    if not (compiled_path / "model.pt").exists():
        ...
        config = Qwen3VLInferenceConfig(neuron_config, load_config=load_pretrained_config(MODEL_PATH))
        model = NeuronQwen3VLForCausalLM(MODEL_PATH, config)
    ...
    try:
        model_config = Qwen3VLInferenceConfig.from_pretrained(MODEL_PATH, neuron_config=neuron_config)
    except:
        model_config = Qwen3VLInferenceConfig(neuron_config, load_config=load_pretrained_config(MODEL_PATH))
    
    try:
        model = NeuronQwen3VLForCausalLM.from_pretrained(COMPILED_MODEL_PATH, config=model_config)
    except:
        model = NeuronQwen3VLForCausalLM(MODEL_PATH, model_config)
