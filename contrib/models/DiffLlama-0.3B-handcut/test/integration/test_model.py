#!/usr/bin/env python3
"""
Integration tests for DiffLlama NeuronX implementation.

This model uses the Differential Transformer architecture with:
- Differential attention (paired head subtraction with learned lambda parameters)
- GQA (32 query heads, 8 KV heads, head_dim=64)
- V transformation: chunk heads into 2 halves, concat along head_dim, repeat
- RMSNorm groupnorm on 2*head_dim features after differential subtraction
- SwiGLU MLP activation
- Llama3 RoPE scaling (frequency-dependent)
...
MODEL_PATH = "/shared/dhwanw2/models/DiffLlama-0.3B-handcut"
COMPILED_MODEL_PATH = "/tmp/neuron_models/diffllama/"
...
def _create_model(compile_if_needed=False):
    """Create and optionally compile the DiffLlama model."""
    neuron_config = DiffLlamaInferenceConfig.get_neuron_config_cls()(
    ...
    if compile_if_needed and not (compiled_path / "model.pt").exists():
...
def compiled_model():
    ...
    model = _create_model(compile_if_needed=True)
...
if __name__ == "__main__":
    print("=" * 80)
    print("DiffLlama Integration Tests")
    print("=" * 80)
