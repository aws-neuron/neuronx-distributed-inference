...
Single forward pass with greedy generation on the full 671B model:

| Prompt | Output | Status |
|--------|--------|--------|
| "The capital of France is" | Paris (coherent 113-token response) | PASS |

The output is coherent and factually correct. Full text begins: *"The capital of France is Paris, which is one of the most important and influential cities in the world..."*
...
With random weights, BF16 rounding in MoE layers causes autoregressive divergence after the first token. This is expected and consistent with other MoE models on Neuron (see Trinity PR for similar observations with MoE v2 NKI blockwise matmul bf16 accumulation).
...
| trn2.3xlarge | 2 | 2 | PASS* | Mini model only (development) |
| trn2.3xlarge | 4 | 2 | FAIL | Full model doesn't fit on 4 NCs |

*Mini model (1 dense + 1 MoE layer, random weights) for development validation only.
...
Tests: config parsing (8), RoPE (3), router (9), weight conversion (11) = **31 tests**.
