#!/bin/bash
# Start the MiMo-V2.5-Pro FP8 vLLM OpenAI-compatible server in the foreground.
#
...
# bench_mimo_v2.sh calls this script under the hood for its one-shot
# launch + bench + teardown flow.
#
# Recipe: TP=64, moe_tp=1/moe_ep=64, BS=48, continuous batching + bucketing.
# moe_tp=1/moe_ep=64 keeps each expert's weights and blockwise FP8 scales
# intact on a single rank (6 experts/rank for Pro's 384 experts), avoiding
# the per-rank scale collapse that comes from moe_tp=64 when intermediate=2048
# is TP-sharded below the 128-row scale block boundary.
#
# NxDI's TKG path refuses Expert Parallelism with BS < num_experts/top_k
# (384 / 8 = 48), so BS=48 is the smallest working batch size on the FP8
# path. BS=1 single-stream latency is not currently supported on Pro FP8.
...
# Contrib package src. vllm-neuron's registration hook reads this env var
...
# First-time compile of Pro's 384-expert MoE takes ~60 min TKG + ~15 min
# CTE + ~30 min shard; plan for 2 h.
export VLLM_ENGINE_READY_TIMEOUT_S="${VLLM_ENGINE_READY_TIMEOUT_S:-7200}"

echo "=========================================="
echo "Starting MiMo-V2.5-Pro FP8 vLLM server"
echo "=========================================="
echo "  Model path:                   $MODEL_PATH"
echo "  Port:                         $PORT"
echo "  Compiled artifacts:           $NEURON_COMPILED_ARTIFACTS"
echo "  Compile work dir:             $BASE_COMPILE_WORK_DIR"
echo "  NXDI_CONTRIB_MIMO_V2_FLASH_SRC: $NXDI_CONTRIB_MIMO_V2_FLASH_SRC"
echo ""
