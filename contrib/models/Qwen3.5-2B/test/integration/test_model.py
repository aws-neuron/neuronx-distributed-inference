...
"""
Integration tests for Qwen3.5-2B on Neuron (Trn2).

Runs full compile + inference against the HuggingFace Qwen/Qwen3.5-2B weights.
...
Env vars:
    ...
    QWEN35_COMPILED_PATH     Where to write NEFF (default /tmp/qwen35_2b_traced)
...
MODEL_PATH = os.environ.get("QWEN35_MODEL_PATH", "/mnt/nvme/models/Qwen3.5-2B")
...
requires_model_path = pytest.mark.skipif(
    ...
    reason=f"Qwen3.5-2B weights not found at {MODEL_PATH}",
)
...
def compiled_model():
    ...
    if not os.path.exists(neff):
        print(f"[qwen35_2b] compiling → {COMPILED_PATH}")
        m = NeuronQwen35ForCausalLM(MODEL_PATH, inf_config)
        m.compile(COMPILED_PATH)
        del m
        gc.collect()

    print(f"[qwen35_2b] loading ← {COMPILED_PATH}")
...
def test_generation_and_latency(compiled_model, tokenizer):
    ...
    print(f"\n[qwen35_2b] prompt   : {prompt!r}")
    print(f"[qwen35_2b] output   : {text!r}")
    print(f"[qwen35_2b] TTFT     : {ttft:.1f} ms")
    print(f"[qwen35_2b] TPOT     : {tpot:.1f} ms  ({1000.0/max(tpot,1e-6):.2f} tok/s)")
...
def test_accuracy_vs_hf(compiled_model, tokenizer):
    ...
    try:
        ...
        print(f"[qwen35_2b] skipping HF cross-check: {e}")
    ...
    print(f"[qwen35_2b] HF↔Neuron token match: {n_match}/{n_total}")
