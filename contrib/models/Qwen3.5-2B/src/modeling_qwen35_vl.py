    ...
    def generate(
        ...
        **kwargs,
        ...
        if has_vision and self.vision_model_wrapper is not None:
            ...
            # Pad to match the compiled CTE bucket size (not input length).
            # The traced graph expects vision_embeddings and vision_mask with
            # sequence dim == CTE bucket, so query the text model's neuron_config.
            cte_bucket = getattr(
                self.text_config.neuron_config, "seq_len", input_ids.shape[1]
            )
            seq_len = input_ids.shape[1]
            ...
            # Pad to CTE bucket size on the seq dim.
            if position_ids.ndim == 3:
                pids = position_ids.to(torch.int32).contiguous()
                if pids.shape[2] < pad_limit:
                    pad_amount = pad_limit - pids.shape[2]
                    pad_tensor = torch.zeros(
                        (pids.shape[0], pids.shape[1], pad_amount), dtype=torch.int32
                    )
                    pids = torch.cat([pids, pad_tensor], dim=2)
                else:
                    pids = pids[:, :, :pad_limit].contiguous()
                llava_args.append(pids)
        else:
        ...
        def _next_token_from_output(output, temperature, top_p, top_k):
            # NxDI with on_device_sampling puts the sampled token into
            # `output.tokens` and leaves `output.logits` as None.
            # Without on_device_sampling it fills `output.logits` [B, T, V].
            head = None
            if hasattr(output, "tokens") and output.tokens is not None:
                ...
                head = output[0]
            if head is None:
                print(f"[VL] output type: {type(output).__name__}")
                if hasattr(output, "keys"):
                    for k in list(output.keys())[:16]:
                        v = output[k]
                        print(f"[VL]   {k} -> type={type(v).__name__} "
                              f"shape={getattr(v, 'shape', None)}")
                raise RuntimeError("model output has no tokens/logits")
            # Sampled token from on_device_sampling: shape [B], [B, 1], or [1, B]
            if head.ndim == 3:  # logits [B, T, V]
                return self._sample_token(head[:, -1, :], temperature, top_p, top_k)
            if head.ndim == 2:  # [B, 1] or [1, B]
                if head.shape[-1] == 1:
                    return head.squeeze(-1)
                # [1, B] shape from on_device_sampling
                return head.reshape(-1)[:1] if head.shape[0] == 1 else head[:, 0]
            if head.ndim == 1:  # [B] sampled token
                return head
            if head.ndim == 3:  # logits [B, T, V]
                return self._sample_token(head[:, -1, :], temperature, top_p, top_k)
            raise RuntimeError(f"Unexpected output head shape {tuple(head.shape)}")
        ...
        next_token = _next_token_from_output(output, temperature, top_p, top_k)
