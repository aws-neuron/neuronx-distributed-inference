...
from nki import jit as nki_jit  # NKI 0.3.0 GA (SDK 2.29.1+)
...
# Number of positional arguments NxDI's ModelWrapper passes to the traced model forward()
# before any model-specific custom args. Positions 0-20 are NxDI base args (input_ids,
# attention_mask, position_ids, seq_ids, KV cache pointers, etc.). Our model appends:
#   position 21: mrope_position_ids  (3D multimodal rotary positions for CTE)
#   position 22: vision_embeddings   (padded vision encoder output)
#   position 23: vision_mask         (scatter indices for vision embedding injection)
_NXDI_BASE_FORWARD_ARGS = 21

# Sentinel value for vision_mask padding entries. Must be larger than any real bucket size
# so that pad_inputs() clamps it to padded_seq_len-1 (always a throw-away pad slot).
# Using a large value prevents the scatter from accidentally corrupting the last real token
# when vision content exactly fills the bucket (the "grid 64" issue).
_VISION_MASK_PAD_SENTINEL = 2**30
...
class Qwen35ModelWrapper(ModelWrapper):
    ...
    def pad_inputs(self, *args, pad_type="first_fit"):
        ...
        orig_mrope = (
            args[_NXDI_BASE_FORWARD_ARGS]
            if len(args) >= _NXDI_BASE_FORWARD_ARGS + 1
            else None
        )
        orig_vis_emb = (
            args[_NXDI_BASE_FORWARD_ARGS + 1]
            if len(args) >= _NXDI_BASE_FORWARD_ARGS + 2
            else None
        )
        orig_vis_mask = (
            args[_NXDI_BASE_FORWARD_ARGS + 2]
            if len(args) >= _NXDI_BASE_FORWARD_ARGS + 3
            else None
        )
            ...
            if is_cte:
                ...
                if (
                    ...
                    pad_mask = torch.full(
                        ...
                        fill_value=_VISION_MASK_PAD_SENTINEL,
                        dtype=torch.int32,
                    )
                ...
                padded_args = (
                    *padded_args[:_NXDI_BASE_FORWARD_ARGS],
                    mrope_position_ids,
                    vision_embeddings,
                    vision_mask,
                )
