#!/usr/bin/env python3
"""
Integration tests for OLMo-2-1124-7B NeuronX implementation.
"""
...
from modeling_olmo2 import NeuronOlmo2ForCausalLM, Olmo2InferenceConfig
...
def compiled_model():
    ...
    if not (compiled_path / "model.pt").exists():
        ...
        config = Olmo2InferenceConfig(neuron_config, load_config=load_pretrained_config(MODEL_PATH))
    ...
    try:
        model_config = Olmo2InferenceConfig.from_pretrained(MODEL_PATH, neuron_config=neuron_config)
    except:
        model_config = Olmo2InferenceConfig(neuron_config, load_config=load_pretrained_config(MODEL_PATH))
    
    try:
        model = NeuronOlmo2ForCausalLM.from_pretrained(COMPILED_MODEL_PATH, config=model_config)
    except:
        model = NeuronOlmo2ForCausalLM(MODEL_PATH, model_config)
