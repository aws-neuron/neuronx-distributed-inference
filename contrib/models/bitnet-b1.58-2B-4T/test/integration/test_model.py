#!/usr/bin/env python3
"""
Integration tests for bitnet-b1.58-2B-4T NeuronX implementation.

This model uses the BitNet architecture with:
- 30 layers, Llama-variant with ternary quantized weights (1.58 bits)
- attn_sub_norm (RMSNorm before o_proj, gamma fused into weights)
- ffn_sub_norm (RMSNorm before down_proj, gamma fused into weights)
- ReLU squared (relu2) activation
...
MODEL_PATH = "/shared/dhwanw2/models/bitnet-b1.58-2B-4T-full/"
COMPILED_MODEL_PATH = "/tmp/neuron_models/bitnet-b1.58-2B-4T/"
...
if __name__ == "__main__":
    print("=" * 80)
    print("bitnet-b1.58-2B-4T Integration Tests")
    print("=" * 80)

    compiled_path = Path(COMPILED_MODEL_PATH)
    neuron_config = NeuronConfig(
        tp_degree=2, batch_size=1, seq_len=256,
        max_context_length=256, torch_dtype=torch.bfloat16,
    )
    config = BitNetInferenceConfig.from_pretrained(
        MODEL_PATH, neuron_config=neuron_config,
    )
