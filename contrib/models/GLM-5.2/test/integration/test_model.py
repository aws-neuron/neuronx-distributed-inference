#!/usr/bin/env python3
"""
Integration tests for GLM-5.2 (zai-org/GLM-5.2-FP8) on NeuronX Distributed Inference.

Requirements:
- trn2.48xlarge instance with pre-compiled model and pre-sharded weights
  (run examples/compile.py first)
- nkilib routing fork installed in editable mode (see README Prerequisites)
- UNSAFE_FP8FNCAST=1

Environment variables:
  MODEL_PATH: Path to GLM-5.2-FP8 HuggingFace checkpoint (default: ~/GLM-5.2-FP8)
  COMPILED_MODEL_PATH: Path to compiled model with NEFFs + weights (default: ~/glm52_compiled)
...
def create_model_and_load(compiled_path: str, model_path: str):
    ...
    def load_config(c):
        for k, v in hf_config.items():
            setattr(c, k, v)
        c.dsa_enabled = False  # full MLA (DSA indexer is no-op for seq_len <= 2048)
