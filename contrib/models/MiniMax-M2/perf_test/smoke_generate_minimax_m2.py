#!/usr/bin/env python3
"""Minimal generate smoke test for MiniMax-M2.7 FP8 on Trn2.

Assumes the compiled NEFF already exists at MINIMAX_M2_COMPILED_PATH
(from smoke_compile_minimax_m2.py). Rebuilds the same MoENeuronConfig /
...
MODEL_PATH = os.environ.get(
    ...
    "/opt/dlami/nvme/models/MiniMax-M2.7-Neuron-FP8",
)
COMPILED_PATH = os.environ.get(
    "MINIMAX_M2_COMPILED_PATH",
    "/opt/dlami/nvme/compiled/minimax_m2_tp64_moetp1_ep64_fp8/",
)

# Must match smoke_compile_minimax_m2.py exactly, else load() sees a
# mismatched NEFF.
...
# smoke_compile_minimax_m2.py so load() under the same COMPILED_PATH
...
def main():
    ...
    # world_size = tp_degree; see smoke_compile_minimax_m2.py comment).
