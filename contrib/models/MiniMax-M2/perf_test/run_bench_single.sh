...
#
# Unlike bench_minimax_m2.sh this script does NOT launch or kill the vLLM
# server — you bring your own. That makes it convenient when the bench driver
# in bench_minimax_m2.sh times out during first-time compilation: the server
...
#   CONFIG_NAME=bs256_tp1_ep64_opt CONCURRENCY=32 NUM_PROMPTS=128 bash run_bench_single.sh
#
# Environment knobs:
#   PORT             vLLM server port (default 8000)
#   MINIMAX_M2_PATH  Path to the BF16 checkpoint (default
#                    /opt/dlami/nvme/models/MiniMax-M2-BF16)
...
#   RESULTS_DIR      Where to dump per-run log (default /tmp/bench_results/minimax_m2)
...
RESULTS_DIR="${RESULTS_DIR:-/tmp/bench_results/minimax_m2}"

mkdir -p "$RESULTS_DIR"

echo "=========================================="
echo "MiniMax-M2 single-run benchmark"
echo "=========================================="
...
if ! curl -sf "http://localhost:$PORT/health" > /dev/null; then
    ...
    echo "Start it first (e.g., bench_minimax_m2.sh) and wait until"
