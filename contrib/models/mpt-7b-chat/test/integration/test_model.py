#!/usr/bin/env python3
"""
Integration tests for MPT NeuronX implementation.

This model uses the MPT architecture with:
- ALiBi (Attention with Linear Biases) instead of positional embeddings
- Fused QKV projection (Wqkv) in HF checkpoint, split during weight conversion
...
MODEL_PATH = "/shared/dhwanw2/models/mpt-7b-chat/"
...
def test_output_coherence(compiled_model, tokenizer):
    ...
    prompt = "To solve a quadratic equation, you can"
    inputs = tokenizer(prompt, return_tensors="pt", padding=True)
