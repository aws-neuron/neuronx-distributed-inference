"""
Integration test for Ministral-3-3B-Instruct-2512-BF16 on NeuronX.
...
MODEL_PATH = "/home/ubuntu/models/Ministral-3B-text-only"
TP_DEGREE = 4
MAX_MODEL_LEN = 8192
PORT = 8000
...
def test_throughput():
    """Test that throughput meets minimum threshold (highest in family)."""
    base_url = f"http://localhost:{PORT}"
    model_name = requests.get(f"{base_url}/v1/models").json()["data"][0]["id"]
    ...
    assert tok_s > 80, f"Throughput too low: {tok_s:.1f} tok/s (expected >80)"
