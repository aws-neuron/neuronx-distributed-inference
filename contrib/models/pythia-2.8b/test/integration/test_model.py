#!/usr/bin/env python3
"""
Integration tests for pythia-2.8b NeuronX implementation.
"""
...
from modeling_gpt_neox import NeuronGPTNeoXForCausalLM, GPTNeoXInferenceConfig


# Test configuration
MODEL_PATH = "/home/ubuntu/models/pythia-2.8b/"
COMPILED_MODEL_PATH = "/home/ubuntu/neuron_models/pythia-2.8b/"
...
def create_model_for_inference(compiled_path: str, model_path: str):
    ...
    try:
        model_config = GPTNeoXInferenceConfig.from_pretrained(
            model_path, neuron_config=neuron_config,
        )
    except (TypeError, AttributeError):
        model_config = GPTNeoXInferenceConfig(
            neuron_config, load_config=load_pretrained_config(model_path),
        )
    
    model = NeuronGPTNeoXForCausalLM(model_path, model_config)
...
if __name__ == "__main__":
    print("="*80)
    print("pythia-2.8b Integration Tests")
    print("="*80)
