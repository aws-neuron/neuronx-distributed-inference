#!/usr/bin/env python3
"""
Integration tests for recurrentgemma-2b-it NeuronX implementation.
"""
...
from modeling_recurrent_gemma import NeuronRecurrentGemmaForCausalLM, RecurrentGemmaInferenceConfig

# Test configuration
MODEL_PATH = "/home/ubuntu/models/recurrentgemma-2b-it/"
COMPILED_MODEL_PATH = "/home/ubuntu/neuron_models/recurrentgemma-2b-it/"
...
def compiled_model():
    ...
    if not (compiled_path / "model.pt").exists():
        ...
        config = RecurrentGemmaInferenceConfig(neuron_config, load_config=load_pretrained_config(MODEL_PATH))
        model = NeuronRecurrentGemmaForCausalLM(MODEL_PATH, config)
    ...
    try:
        model_config = RecurrentGemmaInferenceConfig.from_pretrained(MODEL_PATH, neuron_config=neuron_config)
    except:
        model_config = RecurrentGemmaInferenceConfig(neuron_config, load_config=load_pretrained_config(MODEL_PATH))
    
    try:
        model = NeuronRecurrentGemmaForCausalLM.from_pretrained(COMPILED_MODEL_PATH, config=model_config)
    except:
        model = NeuronRecurrentGemmaForCausalLM(MODEL_PATH, model_config)
...
if __name__ == "__main__":
    print("recurrentgemma-2b-it Integration Tests")
    print("="*80)
    # Run tests...
