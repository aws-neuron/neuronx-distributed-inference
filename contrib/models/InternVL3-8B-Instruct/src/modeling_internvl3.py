...
class InternVL3InferenceConfig(ImageToTextInferenceConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Wrap load_config to map InternVL's llm_config -> text_config
        # The NxDI base class calls load_config(self) which sets attributes from
        # config.to_dict(). InternVL HF config has "llm_config" not "text_config",
        # so we intercept to create text_config from llm_config before validation.
        if load_config is not None:
            original_load_config = load_config

            def _patched_load_config(config_obj):
                original_load_config(config_obj)
                # Map llm_config -> text_config if needed
                if (
                    not hasattr(config_obj, "text_config")
                    or config_obj.text_config is None
                ):
                    llm_cfg = getattr(config_obj, "llm_config", None)
                    if llm_cfg is not None:
                        # llm_config can be a dict, PretrainedConfig, or SimpleNamespace
                        if isinstance(llm_cfg, dict):
                            text_config_dict = dict(llm_cfg)
                        elif hasattr(llm_cfg, "to_dict"):
                            text_config_dict = llm_cfg.to_dict()
                        elif hasattr(llm_cfg, "__dict__"):
                            text_config_dict = dict(vars(llm_cfg))
                        else:
                            text_config_dict = {
                                k: getattr(llm_cfg, k)
                                for k in dir(llm_cfg)
                                if not k.startswith("_")
                            }
                        # Set HF defaults needed by NxDI
                        text_config_dict.setdefault("output_attentions", False)
                        text_config_dict.setdefault("output_hidden_states", False)
                        config_obj.text_config = text_config_dict
                        # Also propagate text config keys to top level
                        for key in INTERNVL3_TEXT_CONFIG_KEYS:
                            if key in text_config_dict and not hasattr(config_obj, key):
                                setattr(config_obj, key, text_config_dict[key])

            load_config = _patched_load_config
...
class NeuronInternVL3ForCausalLM(NeuronBaseForImageToText):
    ...
    def forward(
        ...
        if (
            ...
            # The NEFF is compiled for batch=1 (single image). For multi-image
            # requests, loop over images and concatenate vision embeddings.
            num_images = pixel_values.shape[0]
            pv_dtype = self.vision_config.neuron_config.torch_dtype
            if num_images == 1:
                vision_embeddings = self.vision_encoder_model(pixel_values.to(pv_dtype))
            else:
                # Each NEFF call returns [1, text_seq_len, hidden] with the first
                # tokens_per_image positions containing real embeddings (rest are
                # zero-padded by pad_to_text_seq_len inside the NEFF).
                # Extract real tokens from each call, concatenate, re-pad.
                tokens_per_image = 256  # (448/14)^2 * downsample_ratio^2
                emb_list = []
                for i in range(num_images):
                    emb_i = self.vision_encoder_model(
                        pixel_values[i : i + 1].to(pv_dtype)
                    )
                    # Extract real tokens only: [1, tokens_per_image, hidden]
                    emb_list.append(emb_i[:, :tokens_per_image, :])
                # Concatenate: [1, num_images * tokens_per_image, hidden]
                vision_embeddings = torch.cat(emb_list, dim=1)
                # Pad to CTE bucket size (pad_limit)
                total_vis_tokens = vision_embeddings.shape[1]
                if total_vis_tokens < pad_limit:
                    pad_zeros = torch.zeros(
                        1,
                        pad_limit - total_vis_tokens,
                        vision_embeddings.shape[2],
                        dtype=vision_embeddings.dtype,
                        device=vision_embeddings.device,
                    )
                    vision_embeddings = torch.cat([vision_embeddings, pad_zeros], dim=1)
                elif total_vis_tokens > pad_limit:
    ...
    def load_hf_model(model_path, **kwargs):
        """Load original HF InternVL3 model for CPU reference inference.

        Uses FP32 to avoid -inf logits that occur with BF16,
        which cause false failures in logit_validation.
        """
        ...
        # HF generate() requires img_context_token_id to be set
