...
from models.siglip.modeling_siglip import NeuronSiglipConfig, SiglipInferenceConfig, NeuronSiglipMLP
...
def test_mlp_layer(monkeypatch, base_compiler_flags, tolerances, compiler_flags) -> None:
...
def test_nxdi_mlp_vs_transformers_implementation(random_seed) -> None:    
    batch_size, seq_len = 2, 32
    inputs_dtype = model_dtype = torch.float32

    x = torch.randn(batch_size, seq_len, hf_config.hidden_size).to(dtype=inputs_dtype)
    ...
    reference_model = SiglipMLP(config=hf_config).to(dtype=model_dtype)
