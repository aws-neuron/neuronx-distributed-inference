...
from scripts.benchmark import benchmark_sampling
...
# Setting paths
BASE_PATH = os.getenv('PROJECT_HOME', '/home/ubuntu/daanggn-neuron-inference-migration')
DATA_PATH = os.getenv('DATA_HOME', '/home/ubuntu')

# Load config from CONFIG_MODULE environment variable if set
CONFIG_MODULE = os.getenv('CONFIG_MODULE')
if CONFIG_MODULE:
    if not CONFIG_MODULE.startswith('e2e_pipeline.configs.'):
        raise ValueError(f"CONFIG_MODULE '{CONFIG_MODULE}' must start with 'e2e_pipeline.configs.'")
    logger.info(f"Loading config from module: {CONFIG_MODULE}")
    config_module = importlib.import_module(CONFIG_MODULE)  # nosemgrep: python.lang.security.audit.non-literal-import.non-literal-import (CONFIG_MODULE validated by line 39-40)
    CONFIG = config_module.CONFIG
    # Add default values for CTX_BUCKETS and TKG_BUCKETS if not present
    if 'CTX_BUCKETS' not in CONFIG:
        CONFIG['CTX_BUCKETS'] = [CONFIG['SEQ_LENGTH']]
    if 'TKG_BUCKETS' not in CONFIG:
        CONFIG['TKG_BUCKETS'] = [CONFIG['SEQ_LENGTH']]
else:
    raise ValueError("CONFIG_MODULE environment variable must be set")
...
os.environ['NEURON_RT_NUM_CORES']=f"{CONFIG['TEXT_TP_DEGREE']}"
...
def create_neuron_configs():
    ...
    text_config = NeuronConfig(
        ...
        modules_to_not_convert=[
            ...
            "vision_tower",
            *[f"language_model.model.layers.{l}.self_attn" for l in range(hf_config.num_hidden_layers)], 
            ...
            *[f"layers.{l}.self_attn" for l in range(hf_config.num_hidden_layers)], 
            "lm_head",
            ],
...
def run_benchmarks(model, generation_config, num_runs=10, benchmark_inputs=None):
    """Run performance benchmarks for text-only and text+image scenarios."""
    ...
    benchmark_sampling(
        ...
        target="all", 
        image=None,
        benchmark_report_path="benchmark_report_text_only.json", 
        num_runs=num_runs,
        **benchmark_inputs
    )
...
def run_gemma3_generate_image_to_text(run_test_inference=False, run_benchmark=False):
    ...
    if run_benchmark:
        ...
        model.neuron_config.max_new_tokens = 100
        run_benchmarks(model, generation_config, num_runs=5, benchmark_inputs=benchmark_inputs)
