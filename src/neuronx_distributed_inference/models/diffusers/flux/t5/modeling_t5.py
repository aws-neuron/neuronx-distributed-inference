...
class NeuronT5DenseActDense(nn.Module):
    def __init__(self, config: InferenceConfig):
        super().__init__()
        self.wi = ColumnParallelLinear(config.d_model, config.d_ff, bias=False, gather_output=False)
...
class NeuronT5LayerFF(nn.Module):
    def __init__(self, config: InferenceConfig):
        super().__init__()
...
class NeuronT5Attention(nn.Module):
    def __init__(self, config: InferenceConfig, has_relative_attention_bias=False):
        super().__init__()
        ...
        self.q = ColumnParallelLinear(self.d_model, self.inner_dim, bias=False, gather_output=True)
        self.k = ColumnParallelLinear(self.d_model, self.inner_dim, bias=False, gather_output=True)
        self.v = ColumnParallelLinear(self.d_model, self.inner_dim, bias=False, gather_output=True)
        self.o = ColumnParallelLinear(self.inner_dim, self.d_model, bias=False, gather_output=True)
...
class NeuronT5Stack(nn.Module):
    ...
    def forward(
        ...
        # T5EncoderModel does not use attention mask
...
class NeuronT5EncoderModel(nn.Module):
    ...
    def __init__(self, config: InferenceConfig):
        ...
        self.encoder = NeuronT5Stack(encoder_config, self.shared)
    ...
    def forward(
        ...
        assert attention_mask is None, "NeuronT5EncoderModel expects attention_mask is None"
        assert head_mask is None, "NeuronT5EncoderModel expects head_mask is None"
...
class T5InferenceConfig(InferenceConfig):
...
class ModelWrapperT5(ModelWrapper):
    ...
    def input_generator(self) -> List[Tuple[torch.Tensor]]:
        model_inputs = torch.zeros((1, 512), dtype=torch.int64)
        inputs = [(model_inputs,)]
        return inputs
...
class NeuronT5Application(NeuronApplicationBase):
    """
    Application class of the encoder model(s).
    """

    _model_cls = NeuronT5EncoderModel
    ...
    def get_model_wrapper_cls(self):
        return ModelWrapperT5

    def forward(self, model_inputs, output_hidden_states=False):
        return [self.models[0](model_inputs)["last_hidden_state"].to(self.dtype)]
