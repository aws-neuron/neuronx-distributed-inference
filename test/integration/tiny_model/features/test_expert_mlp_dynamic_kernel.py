...
class ExpertMLPWrapper(torch.nn.Module):
    def __init__(self, config, world_size):
        ...
        self.expert_mlp = ExpertMLPs(
            ...
            use_shard_on_block_dynamic_while = config.use_shard_on_block_dynamic_while,
            use_shard_on_intermediate_dynamic_while = config.use_shard_on_intermediate_dynamic_while,
...
def compile_neuron_expert_mlp_model(
        sample_inputs,
        checkpoint,
        load_module,
        tp_degree=1,
...
class TestExpertMLP:
    ...
    def _get_model_config(self, hidden_size, n_routed_experts, num_experts_per_tok, intermediate_size, early_expert_affinity_modulation):
        return {
    ...
    def _create_models(self, model_params, tp_degree_1, ep_degree_1, tp_degree_2, ep_degree_2, dtype, world_size, use_shard_on_intermediate_dynamic_while, use_shard_on_block_dynamic_while):
    ...
    @pytest.mark.parametrize("model_params", [
        {"hidden_size": 5120, "intermediate_size": 8192, "n_routed_experts": 64, "seq_len": 10240, "num_experts_per_tok": 1, "early_expert_affinity_modulation": True},
        {"hidden_size": 7168, "intermediate_size": 2048, "n_routed_experts": 64, "seq_len": 10240, "num_experts_per_tok": 4, "early_expert_affinity_modulation": False},
        {"hidden_size": 2048, "intermediate_size": 1024, "n_routed_experts": 64, "seq_len": 10240, "num_experts_per_tok": 1, "early_expert_affinity_modulation": False},
    ])
    ...
    def test_expert_mlp_configurations_shard_on_block_trn2(self, model_params, tp_degree_2, ep_degree_2, bs):
        self.batch_size = bs
    ...
    @pytest.mark.parametrize("model_params", [
        {"hidden_size": 5120, "intermediate_size": 8192, "n_routed_experts": 64, "seq_len": 10240, "num_experts_per_tok": 1, "early_expert_affinity_modulation": True},
        {"hidden_size": 7168, "intermediate_size": 2048, "n_routed_experts": 64, "seq_len": 10240, "num_experts_per_tok": 1, "early_expert_affinity_modulation": False},
        {"hidden_size": 2048, "intermediate_size": 1024, "n_routed_experts": 128, "seq_len": 10240, "num_experts_per_tok": 1, "early_expert_affinity_modulation": False},
    ])
    ...
    @pytest.mark.skipif(get_platform_target() != 'trn2', reason="Test only runs on trn2 platform")
    def test_expert_mlp_configurations_shard_on_intermediate_trn2(self, model_params, tp_degree_1, ep_degree_1, tp_degree_2, ep_degree_2, bs):
        self.batch_size = bs
        if model_params["n_routed_experts"] % ep_degree_2 != 0:
            pytest.skip(f"n_routed_experts ({model_params['n_routed_experts']}) must be divisible by ep_degree ({ep_degree_2})")
