...
from neuronx_distributed_inference.modules.chunked_prefill.paged_cache import (
    load_block_tables,
    transform_block_tables_for_indirect_load,
)
from neuronx_distributed_inference.modules.chunked_prefill.utils import is_power_of_2
...
@pytest.mark.parametrize("dma_skip", [True, False])
@torch.inference_mode()
def test_load_and_transform_block_tables(
    num_tiles, num_blocks_per_tile, q_head_per_kv_head, head_id, dma_skip: bool
) -> None:
    ...
    if dma_skip:
        # pick half of rows to skip
        skip_value = max_num_blocks * 10
        block_tables[1::2, :] = skip_value
